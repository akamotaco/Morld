...
class OldBox(Object):
    ...
    def examine(self):
        """낡은 상자 조사 - Generator 기반 인스턴스 메서드"""
...
class PowerPanel(Object):
    ...
    name = "배전함"
    actions = ["call:toggle:조작"]
    focus_text = {
        "default": "벽에 녹슨 금속 배전함이 설치되어 있다. 커다란 레버 스위치가 내려가 있다. '주의: 고압 전류'라고 적힌 경고문이 희미하게 보인다."
    }

    def toggle(self):
        """배전함 스위치 조작 - Generator 기반 인스턴스 메서드"""
        current = morld.get_prop("flag:power")

        if current > 0:
            morld.clear_prop("flag:power")
            yield morld.dialog(["스위치를 내렸다.", "주변이 다시 어두워졌다."])
        else:
            morld.set_prop("flag:power", 1)
