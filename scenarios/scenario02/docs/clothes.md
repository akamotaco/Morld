...
> **구현 완료** — `assets/base.py: Item.durability`, `combat.py: degrade_durability()`

### 기본 내구도

`Clothing` 기본 클래스에 `durability = 20` 설정. 모든 의류가 상속.

| 카테고리 | durability | 예시 |
|---------|-----------|------|
| 일반 의류 | 20 | 셔츠, 바지, 치마 등 |
| 속옷 | 10 | 브라, 팬티 |
| 외투/코트 | 30 | 두꺼운 코트, 후드 망토 |
| 군용 | 40 | 전술 조끼, 군용 부츠 |
| 누더기 | 5 | 누더기 상의/하의 |
...
파손된 아이템은 인벤토리에 남아있으며, 향후 수리 시스템에서 복구 가능.
`lost_item` 호출 없음 — 아이템이 소멸하지 않음.

### 내구도 감소 경로

| 경로 | 감소량 | 설명 |
|------|--------|------|
| 성추행 옷 찢기 (`harassment.execute_tear`) | 5 | `TEAR_DURABILITY_DAMAGE` |
| 전투 피격 (미래) | 1 | 방어구 내구도 감소 |

### 임시노출 (성추행 들추기)

옷을 찢지 않고 들추는 행위는 내구도에 영향 없음. 대신 `임시노출:상체`/`임시노출:하체` prop으로 노출 상태를 override.
NPC는 think() Tier 4에서 `_check_exposure_recovery()` → 옷매무새 정리 (1분).

상세: [adult-toys.md#9-성추행-시스템](adult-toys.md#9-성추행-시스템)
