...
`세력` prop 기반 3단계 관계 시스템: **적대(-1) / 중립(0) / 우호(1)**.
- 같은 세력 → 우호(1)
- 정의 안 된 이종 세력 → 중립(0)
...
```python
# chapters/chapter_1.py — 세력 관계 등록
_creature_factions = ("야생 동물", "거미", "유적", "기생")
_human_factions = ("방문자", "숲속 저택", "도시")
for _cf in _creature_factions:
    for _hf in _human_factions:
        combat.register_faction_relation(_cf, _hf, -1)    # 생물 vs 인간 전체 적대
combat.register_faction_relation("야생 동물", "거미", -1)  # 야생 동물(늑대/박쥐) ↔ 거미 적대
combat.register_faction_relation("방문자", "숲속 저택", 1) # 플레이어 ↔ 저택 NPC 우호
combat.register_faction_relation("방문자", "도시", 1)      # 플레이어 ↔ 도시 NPC 우호
combat.register_faction_relation("숲속 저택", "도시", 1)   # 저택 ↔ 도시 NPC 우호
```
...
| 종 | `세력` prop | 비고 |
|---|---|---|
| Wolf (늑대) | `"야생 동물"` | Bat과 동일 세력 |
| Bat (박쥐) | `"야생 동물"` | Wolf과 동일 세력 |
| Spider (거미) | `"거미"` | 야생 동물과 적대 |
| Arachne (아라크네) | `"유적"` | |
| Succubus (서큐버스) | `"유적"` | |
| BreastParasiteCreature | `"기생"` | |
| GenitalParasiteCreature | `"기생"` | |

### 관계 매트릭스

| | 방문자 | 숲속 저택 | 도시 | 야생 동물 | 거미 | 유적 | 기생 |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **방문자** | 우호 | **우호** | **우호** | **적대** | **적대** | **적대** | **적대** |
| **숲속 저택** | **우호** | 우호 | **우호** | **적대** | **적대** | **적대** | **적대** |
| **도시** | **우호** | **우호** | 우호 | **적대** | **적대** | **적대** | **적대** |
| **야생 동물** | **적대** | **적대** | **적대** | 우호 | **적대** | 중립 | 중립 |
| **거미** | **적대** | **적대** | **적대** | **적대** | 우호 | 중립 | 중립 |
| **유적** | **적대** | **적대** | **적대** | 중립 | 중립 | 우호 | 중립 |
| **기생** | **적대** | **적대** | **적대** | 중립 | 중립 | 중립 | 우호 |
