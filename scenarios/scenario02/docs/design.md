...
### 저택 그룹 (Region 0)
| Instance ID | Unique ID | 이름 | 역할 | 특징 | 스토리 역할 |
|-------------|-----------|------|------|------|------------|
| 1 | lina | 리나 | 채집 + 빨래 | 활발하고 명랑함, 순수하지만 낯가림 | 내부 공략: 회유 대상 |
| 2 | sera | 세라 | **리더(알파)** + 사냥 + 순찰 | 과묵하고 듬직함, 카리스마 리더 | **내부 장벽**: 협박/신뢰로 공략 |
| 3 | mila | 밀라 | 요리 + 청소 | 다정하지만 은퇴한 전설의 전사 | **외부 장벽**: 전투력 최강 |
...
### 관계 구조 + 장벽 역할
```
저택: 세라(리더/알파) ← 밀라, 리나 (신뢰)
도심: 엘라(리더) ← 유키 (의지)

내부 공략: 세라(장벽) → 협박/약점  |  리나 → 회유  |  밀라 → 은근 지원
외부 공략: 밀라(장벽) → 전투로 돌파 (엘라+유키 합세 필요)

밀라 이중성:
  - 평소: 다정한 가정부, 플레이어에게 호감 (내부 공략 시 방해 안 함)
  - 전투 시: 은퇴한 전설의 전사, 저택 최강 (공격력 12, 방어력 8)
  - 내부에서 공략하면 아군, 외부에서 침공하면 최종 보스
```
...
|------|------|----------|
| 프롤로그 | 도입 | 기존 프롤로그 완료 |
| 챕터 1 | 숲속 저택 — 플레이어가 '알파' 지위 획득 | 알파 달성 |
| 챕터 2+ | 5명 합류 → 거점 이주 (생존 시뮬레이션) | 미정 |

### 챕터 1: 숲속 저택

#### 도입 상황
- 밀라가 쓰러진 주인공을 저택으로 데려옴
- 밀라: 약간의 호감 (겉으로 내색하지 않음)
- 세라: 알파(리더). 외부인 불신, 모욕적 언행, 경계심 강함
- 리나: 순수하지만 낯가림 심함. 외부인 배척, 마음 여는 데 오래 걸림

#### 초기 호감도 (챕터 1 진입 시)

| 캐릭터 | 초기 호감 | 의미 |
|--------|----------|------|
| 세라 | **-20** | 불신, 적대적 ("쓸모 없으면 내쫓는다") |
| 밀라 | **+5** | 약간의 호감 (데려온 당사자, 겉으로 내색 안 함) |
| 리나 | **-5** | 낯가림 (외부인 배척, 피함) |

설정 위치: `chapters/chapter_1.py` `post_restore()` — 첫 진입 시 1회 설정.

#### 첫 만남 (first meet) 호감도 분기

각 캐릭터의 첫 만남 이벤트는 **현재 호감도에 따라 2가지 분기**:

| 캐릭터 | 저호감 (기본 경험) | 고호감 |
|--------|-------------------|--------|
| 세라 | 호감 < 0: 냉담/모욕적 ("쓸모가 없으면 나가라") | 호감 >= 0: 과묵하지만 담담 |
| 밀라 | 호감 < 10: 사무적/거리감 ("세라가 꺼려할 것") | 호감 >= 10: 따뜻/다정 |
| 리나 | 호감 < 0: 낯가림/도망 ("모르는 사람이랑은...") | 호감 >= 0: 밝고 호기심 |

기존 대사는 고호감 분기로 보존. 저호감 분기가 신규 추가.

#### 일일 퀘스트
- 매일 세라가 플레이어에게 심부름(일일 임무) 부여 (7종 중 3개 랜덤)
- 성공 → 신뢰도 소폭 상승 / 실패 → 신뢰도 하락
- 모욕감을 느껴도 꿋꿋이 버텨야 함
- **영구 비활성화**: 세라 호감 >= 0 또는 세라 복종 >= 50 도달 시 일일퀘스트 중단
  - 호감 루트: "이제 네 능력은 인정한다. 알아서 해라."
  - 복종 루트: 더 이상 명령할 처지가 아님

#### 알파 달성 — 두 가지 루트

**신뢰 루트:**
- 일일퀘스트 반복 수행
- 획득 아이템 선물 + 대화로 호감/신뢰 축적
- 충분한 신뢰 → 자연스럽게 리더십 이양

**굴복 루트:**
- 상대의 약점을 캐서 협박, 성적 강제로 복종 수치 축적
- 반드시 단둘이 있을 때만 가능 — 다른 캐릭터에게 들키면 게임오버
- 공략 순서 자유:
  - 세라를 직접 공략 (위험도 높음)
  - 리나 + 밀라 먼저 → 다수결로 세라 강제 굴복 가능
    (세라는 알파지만 리나·밀라에게는 약한 면이 있음)
- 밀라 특수: 플레이어의 행위를 발견해도 눈감아줄 수 있으나, 밀라 자신을 굴복시키기는 어려움

**혼합 가능:** 신뢰 + 굴복을 캐릭터별로 다르게 적용 가능

#### 유키·엘라 합류 (챕터 1 또는 2)
- **설득**: 신뢰/호감 기반, 자발적 합류
- **납치/협박**:
  - 유키를 납치 → 엘라에게 "유키를 데리고 있다" → 엘라 분노하며 따라옴
  - 강제 합류 시 반발 높음, 이후 대우에 따라 호감 회복 가능
- 도시 은신처보다 저택이 객관적으로 나은 환경 → 시간이 지나면 회복 여지 있음

#### 장기 비전 (참고)
- 5명 모두 합류 후 → 더 나은 거점으로 이주
- 점차 생존 시뮬레이션 방향으로 발전
