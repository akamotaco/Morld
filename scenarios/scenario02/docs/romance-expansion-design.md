...
### Phase 2.2b: NPC↔NPC 정사 조우 개입 (2026-04-25 완료)

**배경**: Phase 2.1에서 구현한 3 선택지 (숨는다/물러난다/목격한다)에 개입 2종 추가.
...
#### 그만두게 한다 (`_run_npc_intimacy_intervene_stop`)

**수치 효과**:
- 강제 케이스: 가해자 `반발` +20, 피해자 `호감` +5 (구원자 감사)
- 합의 케이스: 양쪽 `반발` +10, `호감` -3 (방해 불쾌감)

**2차 연쇄**: 가해 NPC 반발 ≥ 70 누적 시 기존 hostility 경로에서 자연 발동 — 후속 이벤트에서 적대/공격 확률 상승.

#### 참여한다 (옵션 C 근사, `_run_npc_intimacy_intervene_join`)

**Phase 4 난교 시스템 전면 도입 없이 선택지 플롯만 성립**:
- 두 NPC 중 택일 UI
- 선택된 NPC와 1:1 `start_romance` 세션 시작
- 미선택 NPC는 자리 이탈 (정사 상태 해제 + 대기 job)

**모드 분기**:
- 강제 + 피해자 선택 → MODE_CONSENSUAL (구출 후 합의)
- 강제 + 가해자 선택 → MODE_FORCED (가담)
- 합의 난입 → MODE_CONSENSUAL

**Phase 4 본격 구현 대기 중**:
- `state["partner_ids"]` 배열화 + 부위 점유 모델 (`점유:{부위}:{유닛}`)
- 다인 액션 카탈로그 (69/파이즈리펠라/교대삽입 등)
- UI 재설계 (복수 파트너 선택 + 부위 할당)

**커밋**: `082716e`. 신규 3 e2e 테스트. 전체 1493/1493 통과.
