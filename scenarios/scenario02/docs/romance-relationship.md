...
### 자제심 침잠 — 함락 루트 (Phase 1.9)

복종이 60을 넘으면 **실질 자제심**이 점차 무너지도록 설계. era 조교 서사
("몸이 먼저 굴복하고 정신이 뒤따른다")를 스탯 동역학으로 반영.

- `get_effective_restraint(unit_id)` 파생 함수 (romance_dynamics.py)
- `effective = raw_restraint - max(0, 복종 - 60) × 0.75`
- 복종 100 → 자제심 -30 감쇠 (최대)
- 적용처: `compute_trance_level`, `get_restraint_modifier` (availability_score)
- **raw prop(`성격:자제심`)은 불변** — 파생만 변화

결과: 함락 NPC는 트랜스 방어와 행위 점수 페널티가 자연히 약해져
자발적 행동/고난도 행위 허용 범위 확장. 순애 루트(복종 낮음)에는 영향 없음.

### 트랜스 상태 (Phase 1.6.1 / 1.8 / 1.9.1)

**`상태:트랜스` prop** (0~100) — 성적 흥분의 통합 축.

```
base = (성욕 + 절정게이지) / 2 + 누적절정경험×0.3 (cap 15)
base × 자제심_방어_factor
+ 트랜스:외부   # 세뇌/약물/최면 + 절정 여운 가산
→ clamp 0~100
```

- 트랜스 ≥ 60 (`TRANCE_ENTRY`): 페르소나 억제 풀림, 자발 행동 진입 조건
- 트랜스 ≥ 80 (`TRANCE_DEEP`): 깊은 침잠, 인간어 상실, 저항 극도 약화

**자제심 방어** (비대칭): 자제심 > 50 구간에서만 감쇠. 100에서 90% 방어.

**관계 동역학과 상호작용**:
- 절정 발동 시 `트랜스:외부` +20 → 연속 절정이 자제심 일시 뚫음 (Phase 1.9.1)
- 1시간 tick으로 자연 회복 (-10/h)
- 트랜스 깊을수록 호감/반발 획득 ×0.3 (관계 기억 약화), 복종/절정/경험 ×1.5 (몸 각인 강화)
- 탈출 확률 -0.15 (entry) / -0.30 (deep) — 저항 의지 약화
