...
props = {
    ...
    "관계:세라:복종": 0,      # 세라의 복종 수치
}
```

### 챕터 1 초기 호감도

`chapter_1.py` `post_restore()` 에서 첫 진입 시 1회 설정:

| 캐릭터 | prop | 초기값 | 의미 |
|--------|------|--------|------|
| 세라 | `관계:세라:호감` | -20 | 불신/적대 |
| 밀라 | `관계:밀라:호감` | +5 | 약간의 호감 |
| 리나 | `관계:리나:호감` | -5 | 낯가림/경계 |

### 세라 일일 퀘스트 시스템

세라가 매일 아침 7종 중 3개를 랜덤 선택하여 플레이어에게 부여.

**퀘스트 풀:** `daily_gather_herb`, `daily_gather_berry`, `daily_firewood`, `daily_fishing`, `daily_clean`, `daily_water_garden`, `daily_deliver_food`

**선택 저장:** `SeraAgent._memory["daily_quest_list"]` (prop 아닌 agent memory — 개인 데이터)

**영구 비활성화 조건:**
- `관계:세라:호감 >= 0` (신뢰 루트: 일일퀘 반복으로 호감 축적)
- `관계:세라:복종 >= 50` (굴복 루트: 더 이상 명령할 처지가 아님)
- 한 번 비활성화되면 다시 활성화되지 않음 (초반 전용)
...
| 그리움 | `그리움:{name}` | 호감 30+ 대상별 축적 | 70 (대화), 80 (선물) | 만남/대화 시 해소 |
| 성욕 | `상태:성욕` | +0.5/h (동적 cap) | — | romance |
| 절정 | `상태:절정` | 삽입물/착용형 효과 - 3/h | 100 도달 | 비로맨스 절정 이벤트 |

> 절정 상시 관리 + 성인용품/결박 상세: [adult-toys.md](adult-toys.md) 참조.
...
그리움(`그리움:{name}`) 기반 NPC 간 자발적 대화 시스템. Tier 4h (목욕 다음).
...
| 쿨다운 | 1시간 |
| 대화 시간 | 30분 (대화 job) |
| 효과 | 양측 그리움 해소 (→ 0) + 평판 전파 |
| 대상 | 가장 그리운 대상 (cross-region, NPC+플레이어) |
