...
from engine.asset_base import (
    DialogueCoverageError,  # noqa: F401 -- U4 승격, 재수출
    TextSelector,
...
# 아키타입 공용 묘사 풀 (U4 승격) — 기존 `from assets.base import ...` 하위호환 재수출
from engine.archetype_describe import (  # noqa: F401
    build_focus_rules, build_describe_rules,
    _compute_bukkake_extra, _SEMEN_EXTERNAL_PARTS,
    _DEFAULT_FOCUS_ORDER, _DEFAULT_DESCRIBE_ORDER,
    _FOCUS_RESTRAINT, _FOCUS_CLIMAX, _FOCUS_EXPOSURE_BODY, _FOCUS_SHAME,
    _FOCUS_PERSONALITY, _FOCUS_SEMEN, _FOCUS_INTERNAL_SEMEN, _FOCUS_MOOD,
    _FOCUS_DESIRE, _FOCUS_AFFECTION, _FOCUS_MENSTRUATION,
    _FOCUS_ASSAULT_VICTIM, _FOCUS_NPC_INTIMACY, _FOCUS_CARRYING,
    _DESCRIBE_RESTRAINT, _DESCRIBE_CLIMAX, _DESCRIBE_EXPOSURE_BODY,
    _DESCRIBE_SHAME, _DESCRIBE_SEMEN, _DESCRIBE_INTERNAL_SEMEN,
    _DESCRIBE_DESIRE, _DESCRIBE_AFFECTION, _DESCRIBE_FATIGUE,
    _DESCRIBE_MENSTRUATION, _DESCRIBE_PARASITE, _DESCRIBE_PARASITE_REACTION,
    _DESCRIBE_ASSAULT_VICTIM, _DESCRIBE_NPC_INTIMACY, _DESCRIBE_MASTURBATION,
    _DESCRIBE_IDLE_COMMON, _DESCRIBE_IDLE_ARCHETYPE, _DESCRIBE_CARRYING,
)
...
class Asset(_EngineAsset):
    ...
    TextSelector/select_text/DialogueCoverageError 와 함께 U4에서 엔진으로
    승격 완료. 이 클래스는 기존 `from assets.base import Asset` 사용처를 위한
    이름 유지용이다.
    """
    pass


class Unit(_EngineUnit):
    """Unit 베이스 (캐릭터/오브젝트 공통) — engine.asset_base.Unit + S02 디버그 액션"""
...
class Character(_CharacterBase):
    ...
    def give_gift(self):
        ...
        if is_food:
            ...
            if random.random() < eat_chance:
                ...
                # 음식 첨가물 효과 일괄 적용 (미약/취기/독주/마약/최면제/배란유도제/정력제)
                from romance_core import apply_food_additive_effects
                apply_food_additive_effects(partner_id, item_id)
    ...
    def should_initiate_skinship(self, player_id: int) -> bool:
        ...
        # Slice P6': 지배 값에 비례한 임계치 완화 (지배 100 시 최대 -20).
        # boolean 전환 없이 지배 축적에 따라 점진적으로 주도 빈도 증가.
        dominance = get_dominance(self.instance_id, player_id)
        dom_relief = int(dominance * 0.2)
        arousal_threshold = max(0, arousal_threshold - dom_relief)
        label_threshold = max(0, label_threshold - dom_relief)
        arousal = props.get("상태:성욕", 0) if props else 0
