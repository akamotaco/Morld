...
def _reset_trade_items(unit_id):
    """
    거래 아이템 리셋 + HP 최대치 회복
...
CHARACTER_REACTIONS = {
    "light": {
        (80, 80, 60): [
            "{name}의 자신만만한 표정이 흔들리고 있다. 귀끝이 빨갛다.",
            "{name}가 시선을 돌리며 입술을 깨물고 있다.",
        ],
        (80, 30): [
            "{name}가 허를 찌르는 접촉에 미세하게 굳었다.",
        ],
        (30, 0): [
            "{name}가 아무렇지 않은 듯 한 발 물러서고 있다.",
        ],
        (-30, 0): [
            "{name}가 손을 거칠게 쳐내고 있다.",
        ],
    },
    "medium": {
        (80, 80, 80): [
            "{name}의 능글맞은 웃음이 사라지고 있다. 숨을 참을 수가 없다.",
        ],
        (80, 80, 60): [
            "{name}가 이를 악물고 있다. 자신만만함이 조금씩 무너지고 있다.",
        ],
        (80, 30): [
            "{name}가 한숨을 내쉬며 천장을 본다.",
        ],
        (30, 0): [
            "{name}가 차갑게 손목을 잡고 있다. \"...선 넘지 마.\"",
        ],
    },
    "strong": {
        (80, 80, 80): [
            "{name}의 자신감이 완전히 무너지고 있다. 떨리는 입술로 무언가를 중얼거리고 있다.",
        ],
        (80, 80): [
            "{name}가 처음 보는 표정을 짓고 있다. 뭔가를 참고 있는 얼굴이다.",
        ],
        (80, 30): [
            "{name}가 조용히 맡기고 있다. 손끝만 떨리고 있다.",
        ],
        (30, 0): [
            "{name}가 차갑게 저항하고 있다.",
        ],
    ...
    "penetration": {
        (80, 80, 100): [
            "{name}의 황금색 눈이 초점을 잃어가고 있다. 자신만만했던 그녀가 아니다.",
        ],
        (80, 80, 80): [
            "{name}가 황금색 눈으로 당신을 올려다보고 있다. 처음 보는 얼굴이다.",
        ],
        (80, 30): [
            "{name}가 이를 악물며 숨을 참고 있다.",
        ],
        (30, 0): [
            "{name}가 차갑게 버티고 있다.",
        ],
...
class Faye(Character):
    ...
    REACTION_PROFILE = {
        ...
        "archetype": "proud",
        "speech_level": "casual",
        "speech_shifts": {"arousal": {"extreme": "pleading"}},
        "vars": {},
        "line_overrides": {},
        "overrides": {},
    ...
    ROMANCE_REACTIONS = {
        ...
        "ejaculation_internal_음부:start": [
            ...
            ({"욕망": 80, "경험:질내사정": 5}, ["...안에 쏟아. 상관없어.", "...또 임신이라도 하는 거야?"]),
    ...
    # 거래 UI
    # ========================================
    def _show_trade_ui(self, context):
        """거래 UI — 페이 인벤토리 기반 아이템 판매"""
        player_id = morld.get_player_id()
        if not player_id:
            yield ui.dialog("...")
            return
...
class FayeAgent(BaseAgent):
    ...
    def think(self):
        ...
        if current_day != self._last_trade_day:
            _reset_trade_items(self.unit_id)
