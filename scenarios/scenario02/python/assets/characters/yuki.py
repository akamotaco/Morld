# assets/characters/yuki.py - 유키 캐릭터 Asset
#
# ============================================================
# 캐릭터 설정
# ============================================================
# 이름: 유키 (Yuki)
# 성별: 여성
# 나이: 불명 (10대 후반~20대 초반으로 추정, 기억 없음)
#
# 외모:
#   - 은발 장발, 붉은 눈
#   - 창백하고 연약해 보이는 인상
#   - 작고 조용한 분위기
#
# 성격:
#   - 수줍음: 낯을 많이 가리고, 먼저 말 걸기 어려워함
#   - 얌전함: 조용하고 차분, 눈에 띄지 않으려 함
#   - 의존적: 엘라에게 깊이 의지함
#   - 내면: 마음을 열면 따뜻하고 순수함, 외로움을 많이 탐
#
# 좋아하는 것:
#   - 책, 독서 (조용히 읽는 시간)
#   - 엘라와 함께 있는 것
#   - 평화롭고 조용한 환경
#   - 따뜻한 곳, 안전한 느낌
#
# 싫어하는 것:
#   - 시끄러운 곳, 사람이 많은 곳
#   - 낯선 사람, 예측 불가능한 상황
#   - 갑작스러운 변화
#   - 혼자 남겨지는 것
#
# 취미:
#   - 독서 (다양한 장르)
#   - 청소, 빨래 (할 수 있는 일을 묵묵히)
#   - 엘라 곁에 있기
#
# 과거사:
#   - 기억 없음. 도심 어딘가에서 눈을 떴음
#   - 엘라에게 발견되어 함께 지내게 됨
#   - 과거에 대한 단서가 없어 불안할 때도
#
# 현재 배경:
#   - 도심 생존자 그룹 (엘라와 둘)
#   - 은신처에서 청소, 빨래 등 보조 역할
#   - 엘라 없이는 밖에 나가기 어려움
#   - 저택 그룹과는 별개로 도심에서 생활
#
# 말투 특징:
#   - 말 끝이 흐려지거나 끊김 ("...네", "...그래요...")
#   - "..."을 자주 사용
#   - 조용하고 작은 목소리
#   - 짧은 문장, 말수가 적음
#
# 관계:
#   - 엘라: 유일하게 의지하는 사람, 깊은 신뢰
#   - 플레이어: 처음엔 경계, 엘라가 인정하면 점차 마음을 열음
#   - 저택 그룹: 잘 모름, 만난 적 없음
#
# ============================================================
# Rule-based 텍스트 선택 시스템 사용
# - TALK_RULES: 대화 규칙
# - DESCRIBE_RULES: 장소에서 보이는 묘사 규칙
# - FOCUS_RULES: 클릭했을 때 상세 묘사 규칙
# ============================================================

import morld
import ui
from assets.base import Character
from think import BaseAgent, register_agent_class

_M = 60_000  # millis per minute


class Yuki(Character):
    unique_id = "yuki"
    name = "유키"
    type = "female"
    sexual_orientation = "bisexual"
    props = {
        "성별": "female", "성적지향": "bisexual",
        "외모:은발": 1, "외모:장발": 1, "외모:붉은눈": 1,
        "성격:수줍음": 1, "성격:얌전함": 1,
        "관계:엘라:의지": 1,
        "나이": 13,
        "상태:성욕": 0, "상태:질투": 0,
        "상태:피로": 0, "상태:기분": 5,
        "can:lie_down": 1,
        "can:sleep": 1,
        "can:bath": 1,
        "can:toggle_switch": 1,
        "생존:체력": 100, "생존:최대체력": 100,
        "생존:포만감": 80, "생존:최대포만감": 100,
        "처녀:구강": 1,
        "처녀:음부": 1,
        "처녀:항문": 1,
        "근력": 3, "체력": 4,
        "체격": 1, "가슴:크기": 1,
    }
    actions = [
        "call:talk:대화",
        "call:errand:심부름#",         # 퀘스트 제안 가능 시만 표시
        "call:give_gift:선물하기",
        "call:romance:스킨십",
        "call:force_romance:강제 행위",
        "call:debug_props:(디버그) 속성 보기#",
        "call:debug_affection_up:(디버그) 호감도 +10#",
        "call:debug_affection_down:(디버그) 호감도 -10#",
        "call:debug_arousal_up:(디버그) 성욕 +20#",
        "call:debug_arousal_down:(디버그) 성욕 -20#",
        "call:debug_submission_up:(디버그) 복종 +20#",
        "call:debug_submission_down:(디버그) 복종 -20#",
        "call:debug_work_order:(디버그) 작업지시#",
        "call:debug_pregnancy_info:(디버그) 임신 정보#",
        "call:debug_force_conceive:(디버그) 강제 임신#",
        "call:debug_force_birth:(디버그) 강제 출산#",
    ]
    mood = []

    # ========================================
    # 대화 주제 목록 (주제 선택 메뉴)
    # ========================================
    TALK_TOPICS = [
        "잡담",
        "본인에 대해",
        "좋아하는 책",
        "평소에 뭐 해?",
    ]

    # ========================================
    # 대화 규칙 (주제별 조건 → 대사 또는 메서드명)
    # ========================================
    TALK_RULES = {
        "잡담": [
            # 특수 상황 (최우선)
            ({"activity": "수면"}, {"pages": ["(자고 있다)", "...zzZ"]}),

            # Activity 기반
            ({"activity": "청소"}, {"pages": ["...청소 중이에요...", "..."]}),
            ({"activity": "정원"}, {"pages": ["...텃밭 돌보고 있어요...", "...싹이 나고 있어요..."]}),
            ({"activity": "빨래"}, {"pages": ["...빨래를 널고 있어요...", "...조금만 기다려 주세요..."]}),
            ({"activity": "식사"}, {"pages": ["(조용히 먹고 있다)", "...맛있어요..."]}),
            ({"activity": "휴식"}, {"pages": ["(책을 읽고 있다)", "...아, 네..."]}),
            ({"activity": "요리"}, {"pages": ["...요리 중이에요...", "...조금만 기다려 주세요..."]}),
            ({"activity": "독서"}, {"pages": ["(책을 읽고 있다)", "...아, 네..."]}),
            ({"activity": "목욕"}, {"pages": ["(목욕 중이다)", "...?!"]}),
            ({"activity": "준비"}, {"pages": ["...지금 준비 중이에요...", "..."]}),

            # 호감도 기반 (진척도 증가 로직 포함)
            ({"호감": 70}, "_talk_friendly_high"),
            ({"호감": 50}, "_talk_friendly_mid"),

            # mood 기반
            ({"mood": "기쁨"}, {"pages": ["...네...", "...(희미하게 웃는다)"]}),
            ({"mood": "슬픔"}, {"pages": ["............", "..."]}),

            # 기본값
            ({}, {"pages": ["...네?", "...무슨 일이세요...?"]}),
        ],

        "본인에 대해": [
            # 특수 상황
            ({"activity": "수면"}, {"pages": ["(자고 있다)", "...zzZ"]}),

            # 진척도별 사적인 대화 (플래그로 일회성 체크)
            ({"호감": 70, "진척도": 3}, "_talk_progress_3"),
            ({"호감": 60, "진척도": 2}, "_talk_progress_2"),
            ({"호감": 50, "진척도": 1}, "_talk_progress_1"),

            # 호감도 기반
            ({"호감": 70}, {"pages": [
                "...저요...?",
                "...유키예요...",
                "...엘라 언니랑 같이 있어요...",
                "...청소하거나... 책 읽거나... 그래요...",
                "...(작게 웃는다)",
            ]}),
            ({"호감": 50}, {"pages": [
                "...유키... 라고 해요...",
                "...여기서 엘라 언니랑 지내고 있어요...",
                "...그게... 전부예요...",
            ]}),
            ({"호감": 30}, {"pages": [
                "...유키예요...",
                "......",
            ]}),

            # 기본값
            ({}, {"pages": ["............", "...나중에... 말씀드릴게요..."]}),
        ],

        "좋아하는 책": [
            # 특수 상황
            ({"activity": "수면"}, {"pages": ["(자고 있다)", "...zzZ"]}),

            # 호감도 기반
            ({"호감": 70}, {"pages": [
                "...책이요...?",
                "...이야기책 좋아해요...",
                "...모험 이야기나... 따뜻한 이야기...",
                "...읽으면... 마음이 편해져요...",
                "...같이 읽을래요...?",
            ]}),
            ({"호감": 50}, {"pages": [
                "...책요...?",
                "...조용히 읽을 수 있는 거... 좋아해요...",
                "...이야기책이요...",
            ]}),
            ({"호감": 30}, {"pages": [
                "...책은... 좋아해요...",
                "......",
            ]}),

            # 기본값
            ({}, {"pages": ["............", "...아직은..."]}),
        ],

        "평소에 뭐 해?": [
            # 특수 상황
            ({"activity": "수면"}, {"pages": ["(자고 있다)", "...zzZ"]}),

            # 호감도 기반
            ({"호감": 70}, {"pages": [
                "...평소에요...?",
                "...청소하거나... 빨래하거나...",
                "...엘라 언니 도와드려요...",
                "...시간 나면... 책 읽어요...",
                "...조용한 게 좋아요...",
            ]}),
            ({"호감": 50}, {"pages": [
                "...평소요...?",
                "...청소하고... 빨래하고...",
                "...그런 거 해요...",
            ]}),
            ({"호감": 30}, {"pages": [
                "...그냥... 이것저것...",
                "......",
            ]}),

            # 기본값
            ({}, {"pages": ["............", "...별거 없어요..."]}),
        ],
    }

    # ========================================
    # Describe 규칙 (장소에서 보이는 묘사)
    # ========================================
    DESCRIBE_RULES = [
        # 이동 중
        ({"is_traveling": True, "activity": "요리"}, "{name}가 부엌으로 향하고 있다."),
        ({"is_traveling": True}, "{name}(이)가 조용히 어딘가로 향하고 있다."),

        # Activity 기반
        ({"activity": "청소"}, "{name}가 조용히 청소하고 있다."),
        ({"activity": "정원"}, "{name}가 조용히 텃밭을 돌보고 있다."),
        ({"activity": "빨래"}, "{name}가 빨래를 널고 있다."),
        ({"activity": "요리"}, "{name}가 조용히 요리하고 있다."),
        ({"activity": "독서"}, "{name}가 조용히 책을 읽고 있다."),
        ({"activity": "식사"}, "{name}가 조용히 식사 중이다."),
        ({"activity": "수면"}, "{name}가 새근새근 잠들어 있다."),
        ({"activity": "휴식"}, "{name}가 책을 읽고 있다."),
        ({"activity": "목욕"}, "{name}가 목욕 중이다."),

        # 위치 기반
        ({"location": (0, 4)}, "{name}가 욕실을 청소하고 있다."),
        ({"location": (0, 1)}, "{name}가 소파 구석에 앉아 책을 읽고 있다."),
        ({"location": (2, 5)}, "{name}가 조용히 앉아 있다."),  # 도심 은신처

        # 정액 묘사 (높은 수치 우선)
        ({"정액:얼굴": 20}, "{name}의 얼굴에 정액이 묻어 있다."),
        ({"정액": 60}, "{name}의 몸에 정액이 흥건하다."),
        ({"정액": 30}, "{name}의 피부에 정액이 묻어 있다."),
        ({"정액": 10}, "{name}의 어딘가가 번들거린다."),

        # 체내정액 묘사
        ({"체내정액:음부": 50}, "{name}의 다리 사이로 하얀 것이 조용히 흘러내리고 있다."),
        ({"체내정액:음부": 1}, "{name}의 아랫배가 미세하게 부풀어 있다."),
        ({"체내정액:구강": 1}, "{name}가 말없이 입을 다물고 있다."),

        # 성욕 기반 (높은 성욕 우선) - 유키: 말없이 다가오는 타입
        ({"성욕": 80}, "{name}가 말없이 곁에 다가와 있다. 귀와 볼이 새빨갛다."),
        ({"성욕": 60}, "{name}가 계속 이쪽을 쳐다본다. 눈을 마주치면 황급히 피한다."),
        ({"성욕": 40}, "{name}가 평소보다 어딘가 안절부절못하고 있다."),

        # 호감도/애정 기반 - 유키: 조용히 곁에 있으려 함
        ({"호감": 80}, "{name}가 살며시 옆에 서 있다. 눈빛이 따뜻하다."),
        ({"호감": 50}, "{name}가 가까이 있다. 존재만으로 안심하는 표정이다."),
        ({"호감": 70}, "{name}가 슬쩍 이쪽을 본다. 고개를 살짝 끄덕인다."),
        ({"호감": 50}, "{name}가 경계하지 않는 눈으로 이쪽을 본다."),

        # 기본값
        ({}, "{name}가 조용히 서 있다."),

        # Phase 7: 체력 저하 묘사
        ({"피로도_체력": 85}, "{name}가 불안한 눈으로 다가온다. \"...daijoubu desu ka...? 얼굴이...\""),
        ({"피로도_체력": 70}, "{name}가 걱정스럽게 이쪽을 본다. \"...안색이 안 좋아요...\""),
    ]

    # ========================================
    # Focus 규칙 (클릭했을 때 상세 묘사)
    # ========================================
    FOCUS_RULES = [
        # 정액 묘사 (부위별)
        ({"정액:얼굴": 10}, "얼굴에 하얀 것이 묻어 있다."),
        ({"정액:가슴": 10}, "가슴에 정액이 묻어 있다."),
        ({"정액:배": 10}, "배에 정액이 흘러내려 있다."),
        ({"정액:음부": 10}, "음부 주변에 정액이 묻어 있다."),
        ({"정액:엉덩이": 10}, "엉덩이에 정액이 묻어 있다."),

        # 체내정액 묘사
        ({"체내정액:음부": 50}, "다리 사이로 조용히 정액이 흘러내리고 있다."),
        ({"체내정액:음부": 1}, "아랫배가 미세하게 부풀어 있다."),
        ({"체내정액:구강": 1}, "말없이 입을 꾹 다물고 있다."),

        # Activity 기반
        ({"activity": "청소"}, "열심히 청소하고 있다."),
        ({"activity": "정원"}, "조용히 텃밭을 가꾸고 있다. 손놀림이 정성스럽다."),
        ({"activity": "빨래"}, "빨래를 정성스럽게 널고 있다."),
        ({"activity": "요리"}, "조용히 재료를 다듬고 있다. 손놀림이 정성스럽다."),
        ({"activity": "독서"}, "책에 빠져 있다. 가끔 작게 미소짓는다."),
        ({"activity": "식사"}, "조용히 음식을 먹고 있다."),
        ({"activity": "수면"}, "새근새근 잠들어 있다. 인형 같다."),
        ({"activity": "휴식"}, "조용히 책을 읽고 있다."),
        ({"activity": "준비"}, "조용히 하루를 준비하고 있다."),

        # mood 기반
        ({"mood": "기쁨"}, "살짝 볼이 붉어지며 희미하게 웃는다."),
        ({"mood": "슬픔"}, "고개를 숙이고 있다. 말을 걸기 어려워 보인다."),

        # 성욕 기반 (높은 성욕 우선) - 유키: 내성적이라 더 티가 남
        ({"성욕": 80}, "얼굴 전체가 붉어져 있다. 입술을 달싹이지만 말이 나오지 않는 듯."),
        ({"성욕": 60}, "평소보다 시선이 자주 마주친다. 볼과 귀가 빨갛다."),
        ({"성욕": 40}, "어딘가 초조해 보인다. 손으로 옷자락을 꼭 쥐고 있다."),

        # 애정/호감도 기반 - 유키: 신뢰와 의지
        ({"호감": 80}, "붉은 눈에 신뢰가 가득하다. 조용하지만 깊은 애정이 느껴진다."),
        ({"호감": 50}, "눈이 마주치자 살짝 미소 짓는다. 마음을 열고 있다."),
        ({"호감": 70}, "당신을 보고 살짝 미소 짓는다. 경계심이 풀렸다."),
        ({"호감": 50}, "눈을 피하지 않는다. 조금씩 마음을 열고 있는 듯하다."),

        # 기본값
        ({}, "은빛 긴 머리의 조용한 소녀. 붉은 눈이 신비로운 느낌을 준다."),
    ]

    # ========================================
    # NPC 주도 설정 (유키: 매우 수줍음 - 높은 임계값)
    # ========================================
    self_comfort_threshold = 90       # 매우 수줍음
    self_comfort_max_length = 150     # 침실/욕실/화장실만 (length=150)

    INITIATIVE_CONFIG = {
        "arousal_threshold": 80,      # 성욕 임계값 (세라보다 높음 - 더 소극적)
        "affection_threshold": 70,    # 호감도 임계값 (세라보다 높음)
        "cooldown_millis": 720 * _M,   # 쿨다운 12시간 (가장 김)
    }

    # NPC 주도 시 허용 액션 필터
    # 유키는 매우 수줍어서 애정 높아도 제한적
    INITIATIVE_ACTION_FILTERS = [
        ({"호감": 98}, ["hug", "deep_kiss", "breast_touch", "genital_touch", "clit_rub", "penis_touch", "penis_rub"]),
        ({"호감": 90}, ["hug", "deep_kiss", "breast_touch", "genital_touch", "penis_touch"]),
        ({"호감": 85}, ["hug", "deep_kiss", "breast_touch"]),
        ({"호감": 60}, ["hug", "deep_kiss"]),
        ({}, ["hug"]),  # 기본: 포옹만
    ]

    # NPC 주도 중 반응 텍스트
    INITIATIVE_REACTIONS = {
        "start": [
            ({"성욕": 80}, ["......", "...(말없이 다가온다)"]),
            ({}, ["......", "...저기요..."]),
        ],
        "during_hug": [
            ({"성욕": 60}, ["유키가 떨리는 숨을 내쉬며 안아온다."]),
            ({}, ["유키가 조용히 안아온다.", "유키의 심장 소리가 들린다."]),
        ],
        "during_deep_kiss": [
            ({"성욕": 70}, ["유키가 거친 숨을 몰아쉬며 키스를 이어간다."]),
            ({}, ["유키가 떨리며 키스하고 있다."]),
        ],
        "escape_fail": [
            ({}, ["...(말없이 붙잡는다)", "...가지 마세요..."]),
        ],
        "satisfied": [
            ({"호감": 60}, ["...좋아해요...", "...행복해요..."]),
            ({}, ["...고마워요...", "..."]),
        ],
        "during_genital_touch": [
            ({"성욕": 80}, ["유키가 떨리는 손으로 당신을 만지고 있다."]),
            ({}, ["유키가 조심스럽게 당신의 아래를 만지고 있다."]),
        ],
        "during_clit_rub": [
            ({"성욕": 90}, ["유키가 눈물을 글썽이며 당신을 자극하고 있다."]),
            ({}, ["유키가 떨리는 손가락으로 당신을 자극하고 있다."]),
        ],
    }

    # ========================================
    # 은신 성공 반응 (유키: 무서워서 움츠러듦)
    # ========================================
    # 유키는 수줍고 겁이 많아서 들킬 뻔한 상황에 크게 무서워함
    STEALTH_REACTIONS = {
        "text": [
            ({"성욕": 50}, ["...으으... 무서웠어요...", "...(몸을 떨고 있다)"]),
            ({"호감": 40}, ["...다행이에요...", "...(작게 안도한다)"]),
            ({}, ["...!", "...(숨을 죽인다)"]),
        ],
        "effects": {"호감": 2},  # 무서워서 더 의지하게 됨
    }

    FRIENDLY_TALK_CONFIG = {
        "high": {
            "dialog": [
                "...안녕하세요...",
                "...기다리고 있었어요...",
                "...같이 있으면... 안심돼요...",
            ],
            "progress_cap": 3,
        },
        "mid": {
            "dialog": [
                "...안녕하세요...",
                "...뭔가 필요하세요...?",
                "...저라도 괜찮으시다면...",
            ],
            "progress_cap": 3,
        },
    }

    PROGRESS_DIALOGS = {
        1: {
            "fallback": ["...네...", "...무슨 일이세요...?"],
            "dialog": [
                "...저요...?",
                "...유키예요...",
                "...특별한 건... 없어요...",
                "...엘라랑 둘이서 여기 살아요...",
                "...엘라는... 좋은 사람이에요...",
                "...저를 돌봐줘요...",
                "...말이 많진 않지만... 다정해요...",
                "...이 은신처도... 엘라가 찾았어요...",
                "...덕분에... 안전하게 지낼 수 있어요...",
            ],
        },
        2: {
            "fallback": [
                "...",
                "...오늘도... 조용하네요...",
                "...좋아요...",
            ],
            "dialog": [
                "...좋아하는 것요...?",
                "...",
                "...책이요...",
                "...글자를 읽으면... 마음이 편해져요...",
                "...다른 세계로 가는 것 같아요...",
                "...",
                "...그리고... 조용한 게 좋아요...",
                "...시끄러운 건... 무서워요...",
                "...여기는... 조용해서 좋아요...",
                "...",
                "...{player_name}씨도... 조용해서... 좋아요...",
                "...(고개를 숙인다)",
            ],
        },
        3: {
            "fallback": [
                "...",
                "...(조용히 창밖을 바라본다)",
                "...",
            ],
            "dialog": [
                "...",
                "...옛날 이야기요...?",
                "...",
                "...기억이... 없어요...",
                "...눈을 떴을 때... 혼자였어요...",
                "...무서웠어요... 너무 무서웠어요...",
                "...아무것도 모르겠고...",
                "...어디로 가야 할지도...",
                "...",
                "...그때 엘라를 만났어요...",
                "...엘라도... 혼자였어요...",
                "...아무것도 기억 못 한대요...",
                "...",
                "...그래서... 같이 있기로 했어요...",
                "...혼자보다... 나으니까...",
                "...{player_name}씨도... 그런 거죠...?",
                "...기억이... 없는 거...",
                "...같이 있으면... 덜 무서워요...",
            ],
        },
    }

    ROOM_PRIVACY_CONFIG = {
        "목욕": {
            "threshold": 70,
            "high": {
                "dialog": ["[유키]", "...나가.", "유키가 조용하지만 단호하게 말했다."],
                "teleport": 1,
                "after": "은신처 한쪽으로 물러났다.",
            },
            "low": {
                "dialog": ["[유키]", "......", "유키의 눈이 위험하게 빛났다."],
                "teleport": 1,
                "after": "은신처 한쪽으로 쫓겨났다.",
            },
        },
        "화장실": {
            "threshold": 0,
            "low": {
                "dialog": ["[유키]", "...!"],
                "teleport": 1,
                "after": "은신처 한쪽으로 쫓겨났다.",
            },
        },
    }

    # ========================================
    # 가벼운 애정 반응 (포커스 메뉴)
    # 유키: 조용하고 수줍은 타입, 눈을 감거나 침묵
    # ========================================
    CASUAL_REACTIONS = {
        "casual_kiss": {
            "flirty": ["...(눈을 감는다)", "...또... 해주세요...", "...(살짝 미소 짓는다)"],
            "addicted": ["...키스... 해주세요...", "...입술... 더...", "...(떨리는 손으로 옷을 잡는다)"],
            "default": ["...앗...", "...(놀라서 굳는다)", "......"],
        },
        "casual_breast": {
            "flirty": ["...거기는...", "...(눈을 감으며) ...괜찮아요...", "......"],
            "addicted": ["...만져주세요...", "...가슴... 이상해요...", "...(당신에게 기댄다)"],
            "default": ["...앗!", "...(가슴을 감싸며) ...저기...", "......"],
        },
        "casual_butt": {
            "flirty": ["...앗...", "...(고개를 숙이며) ...부끄러워요...", "......"],
            "addicted": ["...뒤도... 만져주세요...", "...(떨리며) ...괜찮아요...", "......"],
            "default": ["...!", "...(놀라서 뒤로 물러난다)", "......"],
        },
        "casual_genital": {
            "flirty": ["...거기는...", "...(얼굴을 묻으며) ...안 돼요...", "...(하지만 거부하지 않는다)"],
            "addicted": ["...만져주세요... 거기...", "...이상해요... 더...", "...(다리에 힘이 풀린다)"],
            "default": ["...!", "...(놀라서 다리를 모은다)", "...(고개를 세차게 젓는다)"],
        },
    }

    # 흥분도 단계별 소음 강도 — 중간 정도
    ROMANCE_SOUND_PROFILE = {"levels": [10, 20, 35], "ecstasy": 50}

    # 애정행위 발각 시 반응 (목격자로서) — 유키: 얼어붙음
    ROMANCE_DISCOVERY_REACTIONS = {
        "default": {
            "text": ["......!", "...(굳어버린다)", "...(말을 잃는다)"],
            "exposed_text": ["......!!", "...(얼굴을 가린다)", "...(몸이 굳는다)"],
            "effects": {"호감": -3, "반발": 3},
        },
        "ella": {
            "text": ["...엘라... 언니랑...?", "......", "...(눈이 흔들린다)"],
            "exposed_text": ["...엘라 언니가... 벗은...?", "...(눈이 크게 떠진다)", "...(말을 잃는다)"],
            "effects": {"호감": -8, "반발": 8},
        },
    }

    SELF_COMFORT_DISCOVERY_REACTIONS = {
        "text": [
            ({"호감": 70}, ["[유키]", "...!", "...(몸이 굳는다)", "...", "...(눈물이 흐른다)"]),
            ({}, ["[유키]", "...!", "유키가 움직임을 멈추고 부들부들 떤다.", "...(아무 말도 못 한다)"]),
        ],
        "effects": {"호감": -3},
    }

    # ========================================
    # 선물 선호도
    # ========================================
    GIFT_PREFERENCES = {
        "liked_categories": ["flower", "trinket"],
        "favorite_items": ["flower_dried", "trinket_pretty_stone"],
        "disliked_categories": ["equipment"],
        "favorite_foods": ["drink_herb_tea", "food_mixed_stew"],
    }

    # ========================================
    # 성적 선호 (체위/부위)
    # ========================================
    SEXUAL_PREFERENCES = {
        "preferred_positions": ["face_sitting", "missionary"],
        "preferred_parts": ["M", "V"],
    }

    # ========================================
    # 스킨십 반응 (action:timing → 조건부 대사 리스트)
    # 유키: 수줍고 내성적, 조용히 받아들이지만 속으론 기뻐함
    # ========================================
    ROMANCE_REACTIONS = {
        # 즉시형 행위
        "head_pat:start": [
            ({}, ["...네...", "...(고개를 살짝 숙인다)", "...좋아요...", "...부끄러워요..."]),
        ],
        "cheek_caress:start": [
            ({}, ["...앗...", "...따뜻해요...", "...(볼이 붉어진다)", "...네..."]),
        ],
        "cheek_pinch:start": [
            ({}, ["...앗... 아파요...", "...(눈물이 맺힌다)", "...왜요...?", "...그만해 주세요..."]),
        ],
        "ear_touch:start": [
            ({}, ["...! 거, 거기는...", "...이상해요...", "...(몸을 떤다)", "...앗..."]),
        ],
        "french_kiss:start": [
            ({"미경험:기억:첫키스": 1}, ["...!! ...(얼어붙는다)", "...처, 처음이에요...", "...(심장이 터질 것 같아요...)"]),
            ({"경험:총만남횟수": 10}, ["...으응... 키스... 좋아요...", "...(살짝 눈을 감으며) ...더요..."]),
            ({}, ["...으응...", "...숨이...", "...(눈을 꼭 감는다)", "...더...요..."]),
        ],
        "butt_caress:start": [
            ({}, ["...!! ...", "...부끄러워요...", "...(얼굴이 빨개진다)", "...거기는..."]),
        ],

        # 토글형 행위
        "hug:start": [
            ({"호감": 50}, ["...좋아요... 이대로...", "...안심돼요..."]),
            ({"호감": 80}, ["...따뜻해요...", "...행복해요..."]),
            ({}, ["...앗...", "...(가만히 있는다)", "...네...", "...따뜻해요..."]),
        ],
        "hug:during": [
            ({"성욕": 50}, ["유키가 숨을 거칠게 몰아쉬며 안겨 있다."]),
            ({"성욕": 30}, ["유키의 심장이 빠르게 뛰는 게 느껴진다."]),
            ({"호감": 40}, ["유키가 안심한 표정으로 안겨 있다."]),
            ({}, ["유키가 조용히 안겨 있다.", "유키의 체온이 느껴진다.", "유키가 눈을 감고 있다.", "유키가 작게 떨고 있다."]),
        ],
        "deep_kiss:start": [
            ({"성욕": 40}, ["...으응... 이상해요..."]),
            ({"호감": 30}, ["...(눈을 감는다)"]),
            ({}, ["...네...", "...눈... 감을게요...", "..."]),
        ],
        "deep_kiss:during": [
            ({"성욕": 50}, ["유키가 몽롱한 눈으로 키스에 빠져 있다."]),
            ({"성욕": 30}, ["유키의 숨결이 거칠어진다."]),
            ({}, ["유키와 깊은 키스를 나누고 있다.", "유키가 눈을 꼭 감고 있다.", "유키의 입술이 살짝 떨린다."]),
        ],
        "breast_touch:start": [
            ({}, ["...!! ...", "...거기는...", "...부끄러워요..."]),
        ],
        "breast_touch:during": [
            ({}, ["유키가 고개를 숙이고 있다.", "유키가 작은 신음을 흘린다.", "유키의 귀가 빨갛다.", "유키가 눈물이 맺힌 채 참고 있다."]),
        ],
        "genital_caress": {
            "start": [
                ({"성욕": 60}, ["...앗...", "...거기는... 부끄러워요..."]),
                ({}, ["...!!", "...(몸을 움찔한다)"]),
            ],
        },
        "clit_stimulation": {
            "start": [
                ({"성욕": 70}, ["...으...", "...이상해요..."]),
                ({}, ["...!!!", "...(눈물이 맺힌다)"]),
            ],
        },
        "genital_touch": {
            "start": [
                ({"성욕": 70}, ["......", "...(작은 신음을 흘린다)"]),
                ({}, ["...거기는...", "...(몸을 떤다)"]),
            ],
            "during": [
                ({"성욕": 80}, ["유키가 작은 신음을 참지 못하고 있다.", "유키의 눈에 눈물이 맺혀 있다."]),
                ({}, ["유키가 고개를 숙인 채 몸을 떨고 있다.", "유키가 당신의 옷을 꽉 잡고 있다."]),
            ],
        },
        "clit_rub": {
            "start": [
                ({"성욕": 80}, ["...으으...", "...이상해요... 멈추지..."]),
                ({}, ["......!!", "...(몸이 크게 떨린다)"]),
            ],
            "during": [
                ({"성욕": 90}, ["유키가 눈물을 흘리며 신음하고 있다.", "유키가 당신에게 매달리고 있다."]),
                ({}, ["유키가 입술을 꽉 깨물고 참고 있다.", "유키의 몸이 계속 떨리고 있다."]),
            ],
        },

        # 즉시 액션 (신규)
        "lip_lick:start": [
            ({}, ["...(눈을 동그랗게 뜬다)", "...간지러워요..."]),
        ],
        "neck_kiss:start": [
            ({"성욕": 50}, ["...으... 거기...♡", "...(몸을 움츠린다)"]),
            ({}, ["...앗...", "...(놀란다)"]),
        ],
        "breast_caress:start": [
            ({}, ["...(가만히 있다)", "...(고개를 숙인다)"]),
        ],
        "nipple_stimulation:start": [
            ({"성욕": 70}, ["...으으...!", "...(몸이 떨린다)"]),
            ({}, ["......!", "...(눈물이 맺힌다)"]),
        ],
        "anal_stimulation:start": [
            ({}, ["...안 돼요...!", "...(겁먹은 표정)"]),
        ],
        # 토글 액션 (신규)
        "tongue_play:start": [
            ({"성욕": 50}, ["...(조심스럽게 혀를 내민다)♡", "...음..."]),
            ({}, ["...(망설이다 따라한다)", "..."]),
        ],
        "tongue_play:during": [
            ({"성욕": 70}, ["유키가 작은 소리를 내며 혀를 맞대고 있다.", "유키가 당신의 옷을 꽉 쥐고 있다."]),
            ({}, ["유키가 서툴게 혀를 움직이고 있다.", "유키가 눈을 꼭 감고 있다."]),
        ],
        "butt_squeeze:start": [
            ({}, ["...앗...", "...(몸을 움츠린다)"]),
        ],
        "butt_squeeze:during": [
            ({"성욕": 60}, ["유키가 당신에게 몸을 기대고 있다.", "유키의 귀가 새빨갛다."]),
            ({}, ["유키가 가만히 참고 있다.", "유키가 눈을 피하고 있다."]),
        ],
        "breast_squeeze:start": [
            ({"성욕": 60}, ["...으... 세게 하면...♡", "...(작은 신음)"]),
            ({}, ["......!", "...(움찔한다)"]),
        ],
        "breast_squeeze:during": [
            ({"성욕": 70}, ["유키가 작은 신음을 참지 못하고 있다.", "유키가 당신의 손을 붙잡고 있다."]),
            ({}, ["유키가 입술을 깨물며 견디고 있다.", "유키의 눈에 눈물이 맺혀 있다."]),
        ],
        "breast_suck:start": [
            ({"성욕": 70}, ["...으...! 빨면...♡", "...(당신의 머리를 잡는다)"]),
            ({}, ["...앗...!", "...(깜짝 놀란다)"]),
        ],
        "breast_suck:during": [
            ({"성욕": 80}, ["유키가 당신의 머리를 안고 떨고 있다.", "유키에게서 작은 신음이 새어나온다."]),
            ({}, ["유키가 입술을 깨물며 참고 있다.", "유키가 눈을 꽉 감고 있다."]),
        ],
        "nipple_lick:start": [
            ({"상태:수유": 1}, ["...! ...나와요...", "...(놀란 눈으로 내려다본다)"]),
            ({}, ["...앗...", "...(몸이 움찔한다)"]),
        ],
        "nipple_suck:start": [
            ({"상태:수유": 1}, ["...빨면... 나와요...", "...(눈물이 글썽이며) ...모유가..."]),
            ({}, ["...으...!", "...(입술을 깨문다)"]),
        ],
        "nipple_suck:during": [
            ({"상태:수유": 1}, ["유키가 조용히 당신의 머리를 안고 젖을 먹이고 있다.", "...(작은 목소리로) ...많이 나와요..."]),
            ({}, ["유키가 떨면서 당신의 머리를 잡고 있다.", "유키가 눈을 꽉 감고 소리를 참고 있다."]),
        ],
        "clit_lick:start": [
            ({"성욕": 80}, ["...앗...!! ♡", "...거기... 핥으면..."]),
            ({}, ["......!!!", "...(겁먹은 표정)"]),
        ],
        "clit_lick:during": [
            ({"성욕": 90}, ["유키가 다리를 떨며 작은 비명을 내고 있다.", "유키가 시트를 꽉 움켜쥐고 있다."]),
            ({}, ["유키가 입술을 깨물며 소리를 참고 있다.", "유키의 눈에서 눈물이 흐르고 있다."]),
        ],
        "cunnilingus:start": [
            ({"성욕": 80}, ["...으...! 혀가... 안에...♡", "...(다리가 떨린다)"]),
            ({}, ["...안 돼요...!", "...(눈물이 글썽인다)"]),
        ],
        "cunnilingus:during": [
            ({"성욕": 90}, ["유키가 소리를 참지 못하고 울고 있다.", "유키가 당신의 머리를 꽉 잡고 있다."]),
            ({}, ["유키가 몸을 떨며 견디고 있다.", "유키가 울먹이고 있다."]),
        ],
        "finger_insertion:start": [
            ({"성욕": 80}, ["...으으...!! 손가락이...♡", "...(몸이 경직된다)"]),
            ({}, ["...아파요...!", "...(눈물이 흐른다)"]),
        ],
        "finger_insertion:during": [
            ({"성욕": 90}, ["유키가 당신의 팔을 잡으며 떨고 있다.", "유키에게서 울음 섞인 신음이 나온다."]),
            ({}, ["유키가 이를 악물며 견디고 있다.", "유키가 당신의 손을 꽉 잡고 있다."]),
        ],
        "fellatio:start": [
            ({"성욕": 70}, ["...(겁먹은 표정으로 조심스럽게 입을 벌린다)", "...이렇게...?"]),
            ({}, ["...(불안한 눈으로 올려다본다)", "..."]),
        ],
        "fellatio:during": [
            ({"성욕": 80}, ["유키가 조심스럽게 해주고 있다.", "유키가 불안한 눈으로 때때로 올려다본다."]),
            ({}, ["유키가 서툴게 움직이고 있다.", "유키가 눈을 감고 하고 있다."]),
        ],
        # 삽입 행위
        "vaginal_penetration:start": [
            ({"성욕": 80}, ["...으으...!! ♡", "...(눈물을 흘리며 당신을 안는다)"]),
            ({}, ["...앗...!!", "...아파요..."]),
        ],
        "vaginal_penetration:during": [
            ({"크기통증": 1}, ["유키가 이를 물며 참는다. \"...크다...\"", "\"...아프지만... 괜찮다...\""]),
            ({"성욕": 90}, ["유키가 당신에게 매달리며 작은 소리로 울고 있다.", "유키의 안이 뜨겁게 조여온다."]),
            ({}, ["유키가 이를 악물며 견디고 있다.", "유키의 눈에서 눈물이 흐르고 있다."]),
        ],
        "receive_penetration:start": [
            ({"성욕": 80}, ["...(조용히 받아들인다)♡", "...(눈물이 맺힌다)"]),
            ({}, ["...(겁먹은 표정)", "..."]),
        ],
        "receive_penetration:during": [
            ({"성욕": 90}, ["유키가 작은 소리를 내며 움직이고 있다.", "유키가 당신의 손을 꽉 잡고 있다."]),
            ({}, ["유키가 눈을 감고 있다.", "유키가 가만히 받아들이고 있다."]),
        ],
        "anal_penetration:start": [
            ({}, ["...안 돼요...!!", "...(눈물이 흐른다)"]),
        ],
        "anal_penetration:during": [
            ({"크기통증": 1}, ["유키가 이를 꽉 문다. \"...뒤는... 더 힘들다...\""]),
            ({"성욕": 90}, ["유키가 소리 없이 울며 떨고 있다.", "유키가 당신의 손을 꽉 잡고 있다."]),
            ({}, ["유키가 입술을 깨물며 참고 있다.", "유키의 몸이 떨리고 있다."]),
        ],
        "receive_anal:start": [
            ({}, ["...(겁먹은 눈)", "...아파요..."]),
        ],
        "receive_anal:during": [
            ({"성욕": 90}, ["유키가 낯선 감각에 작은 비명을 내고 있다.", "유키가 당신에게 매달리고 있다."]),
            ({}, ["유키가 울며 견디고 있다.", "유키의 눈물이 멈추지 않는다."]),
        ],

        # 첫경험 반응
        "first_vaginal_penetration:start": [
            "......아파요...",
            "...처음이에요... 무서워요...",
            "...으으... 찢어지는 것 같아요...",
        ],
        "first_finger_insertion:start": [
            "...으... 손가락이...",
            "...처음이에요... 무서워요...",
            "......이상해요...",
        ],
        "first_anal_penetration:start": [
            "...거기는...! 안 돼요...!",
            "...아파요... 뒤는... 처음이에요...",
            "...으으... 무서워요...",
        ],
        "first_fellatio:start": [
            "...입에... 넣는 거예요...?",
            "...처음이에요... 어떻게 해야...",
            "...(불안한 눈으로 올려다보며 입을 벌린다)",
        ],

        # 사정 참기 반응
        "hold_back_success:start": [
            ({}, ["유키가 고개를 끄덕인다. \"...잘 참았다.\"", "\"...\" 유키가 안도의 숨을 내쉰다."]),
        ],
        "hold_back_failure:start": [
            ({}, ["유키가 눈을 크게 뜬다. \"...안에...?\"", "\"...참지 못했나.\" 유키가 조용히 말한다."]),
        ],

        # 내부 사정 반응
        "ejaculation_internal_음부:start": [
            ({"욕망": 80, "경험:질내사정": 5}, ["...안에... 더... 채워줘요...", "...괜찮아요... 안에 쏟아도..."]),
            ({"경험:질내사정": 3}, ["...또... 안에...", "...이제... 놀라지 않아요..."]),
            ({}, ["...안에......", "...뜨거워요... 안에서...", "...아기... 생기면... 어떡해요..."]),
        ],
        "ejaculation_internal_항문:start": [
            "...뒤에... 나왔어요...?",
            "...이상한 느낌이에요......",
            "...으... 뒤쪽이...",
        ],
        "ejaculation_internal_구강:start": [
            "...으읍...!",
            "...(놀란 눈으로 입을 막는다) ...삼킬 수...",
            "...입안이... 뜨거워요...",
        ],

        # 외부 사정 반응
        "pull_out_얼굴:start": [
            "......얼굴에......",
            "...(눈을 꼭 감는다) ...뜨거워요...",
            "...얼굴이... 더러워졌어요...",
        ],
        "pull_out_가슴:start": [
            "...가슴에......",
            "...뜨거워요... 가슴이...",
            "...(고개를 숙이며 바라본다) ...하얗게...",
        ],
        "pull_out_배:start": [
            "...배에... 나왔어요...",
            "...뜨거워요... 흘러내려요...",
            "...배가... 미끌거려요...",
        ],
        "pull_out_음부:start": [
            "...밖에... 나온 거예요...?",
            "...거기에... 뜨거운 게... 아기는... 괜찮아요...?",
            "...음부에... 묻었어요...",
        ],
        "pull_out_엉덩이:start": [
            "...엉덩이에......",
            "...뒤에... 뜨거운 게...",
            "...흘러내려요...",
        ],
        "pull_out_얼굴_heavy:start": [
            "...많이 나왔네요... 얼굴에 전부...",
            "...눈에 들어왔어요... 앞이 안 보여요...",
            "...뜨거워요... 얼굴에서 뚝뚝 떨어져요...",
        ],
        "pull_out_가슴_heavy:start": [
            "...가슴에 이만큼이나... 많네요...",
            "...뜨거워요... 가슴에서 흘러내려요...",
            "...가슴이 전부 하얗게... 끝나지 않아요...",
        ],
        "pull_out_배_heavy:start": [
            "...많이 나왔네요... 배가 전부...",
            "...뜨거워요... 배꼽에 고여요...",
            "...끝이 없어요... 배에서 흘러내려요...",
        ],
        "pull_out_음부_heavy:start": [
            "...이만큼이... 밖에 나온 거예요...?",
            "...많아요... 다리까지 흘러내려요...",
            "...뜨거워요... 거기에서 멈추지 않아요...",
        ],
        "pull_out_엉덩이_heavy:start": [
            "...엉덩이에 이만큼이나...",
            "...뒤에서 끝이 없어요... 흘러내려요...",
            "...뜨거워요... 엉덩이가 전부...",
        ],

        # 절정 반응
        "ecstasy:start": [
            ({"미경험:기억:첫절정": 1}, ["...!! ...뭐, 뭐예요 이 느낌...?!", "...몸이... 이상해요...", "...(눈물이 글썽이며 떨고 있다)"]),
            ({"경험:절정:V": 10}, ["...또... 가요...♡", "...(조용히 몸을 맡기며) ...으응...♡"]),
            ({}, ["...앗...!! ♡", "...이상해요... 머리가...♡", "...(말없이 떨고 있다)", "...좋아요...♡"]),
        ],
        "ecstasy_V:start": [
            ({}, ["...안에서... 뜨거워요...♡", "...깊은 곳이...♡", "...(배를 누르며 떨고 있다)", "...(말없이 눈물을 흘린다)"]),
        ],
        "ecstasy_M:start": [
            ({}, ["...으...♡", "...(눈이 커진다) ...뜨거워요...", "...(삼키려 한다)", "...(기침하며 떨고 있다)"]),
        ],
        "ecstasy_B:start": [
            ({}, ["...가슴이... 이상해요...♡", "...거기... 만지면...", "...(젖꼭지를 감싸며 떨고 있다)", "...전기... 오는 것 같아요..."]),
        ],
        "ecstasy_P:start": [
            ({}, ["...(손이 떨린다)", "...뜨거워요...♡", "...유키... 때문에...?", "...(조용히 바라보고 있다)"]),
        ],
        "ecstasy_A:start": [
            ({}, ["...뒤에서... 이런...♡", "...안 돼요... 거기는...", "...(믿을 수 없다는 표정으로 떨고 있다)", "...이상해요... 뒤가..."]),
        ],
        "ecstasy_F:start": [
            ({}, ["...앗...♡", "...(눈을 꼭 감는다) ...뜨거워요...", "...얼굴에...", "...(수치스러워 고개를 숙인다)"]),
        ],
        "ecstasy_chain:start": [
            ({}, ["...또...? ...안 돼요...♡", "...연속으로... 무리예요...", "...(온몸을 떨며 매달린다)", "...(말을 잃고 떨고만 있다)"]),
        ],
        "ecstasy_chain_2:start": [
            ({}, ["...또...♡ ...거짓말...이에요...", "...몸이... 멋대로...♡♡", "...(떨리는 손으로 꽉 잡는다) ...무서워요...", "...그래도... 놓지 마세요...♡♡"]),
        ],
        "ecstasy_chain_3:start": [
            ({}, ["...(소리 없이 입을 벌리고 있다)...♡♡♡", "...더... ...더요...♡♡♡", "...(눈이 초점을 잃고 떨고 있다)", "...(작게 중얼거린다) ...좋아요...♡♡♡"]),
        ],
        "ecstasy_intercourse:start": [
            ({}, ["...같이...♡♡", "...(꽉 안으며) ...느껴져요...", "...함께... 이렇게...♡", "...좋아요... 좋아요...♡♡"]),
        ],

        # 정액 삼킴 반응
        "swallow_semen:start": [
            ({"성욕": 70}, ["...(조용히 삼킨다)...♡", "...뜨거워요... 목으로..."]),
            ({}, ["...으읍...", "...(삼키지 못하고 입을 막는다)"]),
        ],
        "swallow_semen_spit:start": [
            ({}, ["유키가 고개를 돌려 뱉는다. \"...삼킬 수 없다...\"", "\"...무리다.\" 유키가 조용히 뱉어낸다."]),
        ],
        "swallow_semen_drip:start": [
            ({}, ["유키의 입에서 정액이 흘러나온다. \"...\"", "\"...아직 못 삼킨다...\" 유키가 입을 연다."]),
        ],
        "swallow_semen_vomit:start": [
            ({}, ["유키가 구역질한다. \"...으...\"", "\"...\" 유키가 고개를 돌려 구역질한다."]),
        ],

        # Phase 5: 거친 행위 반응
        "nipple_pinch:start": [
            ({"성욕": 70}, ["...앗...! itai... demo... 이상해요...♡", "...꼬집으면... 전기가... desu...♡"]),
            ({}, ["...아파요...!", "...꼬집지 마세요... itai..."]),
        ],
        "breast_grab:start": [
            ({"성욕": 70}, ["...앗...! 세게 잡으면... 이상해요...♡", "...거기... 그렇게 하면... sugoi...♡"]),
            ({}, ["...아파요...", "...세게 잡지 마세요..."]),
        ],
        "rough_finger:start": [
            ({"성욕": 70}, ["...거칠게... 하면... 이상해요...♡", "...안 돼요... demo...♡"]),
            ({}, ["...아파요... 살살... onegai...", "...무서워요..."]),
        ],
        "rough_thrust:start": [
            ({"성욕": 70}, ["...으으...! 세게... 오면...♡", "...안쪽까지... sugoi...♡"]),
            ({}, ["...아파요...! 천천히...", "...너무 세요..."]),
        ],
        "rough_thrust:during": [
            ({"성욕": 70}, ["유키가 소리 없이 울면서 당신에게 매달리고 있다.", "유키에게서 억누른 비명이 새어나온다."]),
            ({}, ["유키가 눈물을 흘리며 이를 악물고 있다.", "유키가 떨면서 견디고 있다."]),
        ],
        "hard_anal:start": [
            ({"성욕": 70}, ["...뒤를 세게...? kowai... demo...♡", "...거기는... 이상해요...♡"]),
            ({}, ["...안 돼요...! 아파요...!", "...무서워요... yamete..."]),
        ],
        "hard_anal:during": [
            ({"성욕": 70}, ["유키가 베개에 얼굴을 묻고 떨며 신음하고 있다.", "유키가 울면서도 멈추지 말라는 듯 손을 잡고 있다."]),
            ({}, ["유키가 소리 없이 울며 참고 있다.", "유키가 시트를 꽉 잡으며 견디고 있다."]),
        ],

        # Phase 6: 삽입 중 즉시 액션 반응
        "thrust_deep:start": [
            ({"성욕": 70}, ["...깊어요...! oku made...♡", "...거기 닿으면... 이상해요...♡"]),
            ({}, ["...너무 깊어요...!", "...아파요..."]),
        ],
        "thrust_slow:start": [
            ({"성욕": 70}, ["...천천히... 전부 느껴져요...♡", "...이렇게 하면... kimochi...♡"]),
            ({}, ["...으...♡", "...(눈을 감는다)"]),
        ],
        "grind:start": [
            ({"성욕": 70}, ["...안에서 움직이면... 이상해요...♡", "...거기 누르면... tokeru...♡"]),
            ({}, ["...움직이면... 이상해요...", "..."]),
        ],
        "ejaculate:start": [
            ({"성욕": 70}, ["...뜨거워요... 안에...♡", "...(몸을 떨며 눈을 감는다.)"]),
            ({}, ["...(조용히 몸을 떤다.)", "..."]),
        ],

        # 강제 모드 반응
        "forced_start:start": [
            ({"경험:강제횟수": 5}, ["...(입을 다물고 시선을 돌린다.)"]),
            ({}, ["...그만둬. 후회할 거야."]),
        ],
        "forced_ecstasy:start": [
            ({"경험:강제횟수": 5}, ["...(몸이 반응하는 걸 참으려 하지만...)"]),
            ({}, ["...! (이를 악문다)"]),
        ],
        "forced_break_free:start": [
            ({}, ["...다음에는 가만두지 않을 거야."]),
        ],

        # 트랜스 상태 반응
        "trance:start": [
            ({"성욕": 80}, ["...하아...♡ 더... 해줘요... tomara naide...♡", "...atama ga... 하얘요... 멈추면... 안 돼요...♡"]),
            ({}, ["...으... 몸이... 이상해요...", "...(말없이 떨고 있다.)"]),
        ],
        "trance_insert:start": [
            ({"성욕": 80}, ["...안에... irete... 참을 수 없어요...♡", "...빈 곳이... 아파요... onegai...♡"]),
            ({}, ["...(무의식적으로 허리가 움직이고 있다.)", "...몸이... 제멋대로..."]),
        ],
    }

    # ========================================
    # 이벤트 핸들러
    # ========================================

    def _first_meet_handler(self, player_id):
        """첫 만남 이벤트 핸들러 - 누적형 대화 (Conversation)"""
        # 누적형 대화 빌더 사용
        conv = ui.Conversation("유키")

        # 도입: 유키가 엘라 뒤에 숨어 경계
        conv.narration(
            "은빛 머리카락의 소녀가 있다.",
            "낯선 이의 등장에 경계하는 눈빛을 보낸다.",
            "붉은 눈동자가 차갑게 빛난다.",
            "엘라 뒤에 숨듯 서서 이쪽을 바라본다."
        )

        conv.say(
            "......",
            "...엘라...?"
        )

        conv.narration(
            "유키가 불안한 눈으로 엘라를 올려다본다."
        )

        # 첫 번째 선택지
        conv.ask([
            ("안녕", "hello"),
            ("무서워하지 마", "dont_fear"),
            ("(헤어지기)", "@exit"),
        ])

        conv.respond("hello",
            "...!",
            "......",
            "(유키가 더 깊이 엘라 뒤로 숨는다)"
        )

        conv.respond("dont_fear",
            "......",
            "...무서워...하는 거... 아니야...",
            "...그냥..."
        )

        # 두 번째 선택지
        conv.narration("유키가 조심스럽게 고개를 내민다.")

        conv.ask([
            ("이름이 뭐야?", "name"),
            ("여기서 뭐 해?", "what"),
            ("(헤어지기)", "@exit"),
        ])

        conv.respond("name",
            "...유키...",
            "......",
            "...이름..."
        )

        conv.respond("what",
            "......",
            "...엘라랑... 같이 있어...",
            "...그게 다야..."
        )

        # 마무리
        conv.narration(
            "유키가 다시 엘라 뒤로 숨는다.",
            "하지만 가끔 이쪽을 힐끗거린다."
        )

        # 누적형 대화 시작
        yield conv.end()

        # 시간 경과 처리
        morld.set_npc_time_consume(self.instance_id, "stay", 1 * _M)
        morld.set_npc_job(self.instance_id, "stay", 2 * _M)

        # 첫 만남 완료 처리 (관계:유키:진척도 = 1)
        self.mark_first_meet_done(player_id)

    # ========================================
    # 임신/모드 후유증 반응
    # ========================================

    def _handle_pregnancy_event(self, player_id, event_key):
        """유키 임신 이벤트 반응"""
        import pregnancy as _preg
        week = _preg.get_pregnancy_week(self.instance_id)

        if event_key == "conception:discovery":
            yield ui.dialog(f"[{self.name}]\n\"...아이가... 생겼어요.\"\n\"...당신의... 아이예요.\"")
        elif event_key == "conception:unknown_father":
            yield ui.dialog(f"[{self.name}]\n\"...몸이 이상해요...\"\n\"...무서워요...\"")
        elif event_key == "pregnancy:announcement":
            yield ui.dialog(f"[{self.name}]\n\"...{week}주차... 래요.\"\n\"...잘 키울 수 있을까요...\"")
        elif event_key == "pregnancy:unknown_father":
            yield ui.dialog(f"[{self.name}]\n\"...{week}주차...\"\n\"...아버지가 누군지... 모르겠어요...\"")

    def _handle_mode_aftermath(self, player_id, event_key):
        """유키 모드 피해 후유증 반응"""
        if event_key == "forced_aftermath":
            yield ui.dialog(f"[{self.name}]\n\"......\"\n유키가 아무 말 없이 당신을 바라본다. 눈빛이 얼어붙어 있다.")
        elif event_key == "unconscious_aftermath":
            yield ui.dialog(f"[{self.name}]\n\"...몸이... 이상해요...\"\n유키가 불안한 눈으로 자신의 몸을 내려다본다.")
        elif event_key == "frozen_aftermath":
            yield ui.dialog(f"[{self.name}]\n\"...시간이... 빠진 것 같아요...\"\n유키가 조용히 주변을 둘러본다.")

    # ========================================
    # 침대 이벤트
    # ========================================

    def on_bed_awake(self, bed, player_id, slot, affection, region_id, owner_id):
        """
        유키 침대 반응 (깨어있을 때)
        - 호감도 무관하게 내쫓지 않음 (너무 소심해서 거부 못함)
        - 호감도에 따라 반응만 달라짐
        """
        success = False
        if affection >= 50:
            yield ui.dialog([
                "[유키]",
                "...어...? 같이... 누울 거예요...?"
            ])
            success = morld.sit_on(player_id, bed.instance_id, slot)
            if success:
                yield ui.dialog([
                    "유키의 침대에 누웠다.",
                    "유키가 조용히 자리를 내줬다."
                ])
        elif affection >= 20:
            yield ui.dialog([
                "[유키]",
                "...저기... 그건...",
                "...네... 괜찮아요..."
            ])
            success = morld.sit_on(player_id, bed.instance_id, slot)
            if success:
                yield ui.dialog([
                    "유키의 침대에 누웠다.",
                    "(유키가 움츠리며 구석으로 비켜줬다.)"
                ])
        else:
            yield ui.dialog([
                "[유키]",
                "...!",
                "...저... 그... 네..."
            ])
            success = morld.sit_on(player_id, bed.instance_id, slot)
            if success:
                yield ui.dialog([
                    "유키의 침대에 누웠다.",
                    "(유키가 겁먹은 표정으로 몸을 웅크렸다.)"
                ])

        if not success:
            return

        # 행동 선택지
        lines = "...\n\n"
        lines += "[url=@ret:breast]가슴 만지기[/url]\n"
        lines += "[url=@ret:butt]엉덩이 만지기[/url]\n"
        lines += "[url=@ret:kiss]키스하기[/url]\n"
        lines += "[url=@ret:hug]안아주기[/url]\n"
        if affection >= 50:
            lines += "[url=@ret:romance]스킨십[/url]\n"
        lines += "[url=@ret:nothing]가만히 있기[/url]"
        choice = yield ui.dialog(lines, autofill="off")

        if choice == "nothing" or not choice:
            return

        if choice == "romance":
            from romance import start_romance
            yield from start_romance(player_id, owner_id)
            return

        if affection >= 50:
            if choice == "breast":
                yield ui.dialog([
                    "손을 뻗어 유키의 가슴에 살짝 닿았다.",
                    "[유키]",
                    "+...!!",
                    "+...저, 저기... 그건...",
                    "유키가 새빨개진 얼굴로 눈을 감았다.",
                    "+...하지만 밀어내지는 않았다."
                ])
            elif choice == "butt":
                yield ui.dialog([
                    "손을 뻗어 유키의 엉덩이에 살짝 닿았다.",
                    "[유키]",
                    "+히잇...!",
                    "+...저... 그, 거기는...",
                    "유키가 몸을 떨며 이불을 꽉 잡았다.",
                    "+...거부하지는 못하는 것 같다."
                ])
            elif choice == "kiss":
                yield ui.dialog([
                    "유키의 얼굴에 가까이 다가갔다.",
                    "[유키]",
                    "+...어...? 가, 가까...",
                    "유키의 이마에 가볍게 키스했다.",
                    "유키가 얼굴을 붉히며 눈을 감았다.",
                    "+\"...고마워요...\""
                ])
            elif choice == "hug":
                yield ui.dialog([
                    "유키를 부드럽게 안아줬다.",
                    "[유키]",
                    "+...!",
                    "+...따뜻해요...",
                    "유키가 처음엔 몸을 굳혔지만...",
                    "+이내 조심스럽게 안겨왔다."
                ])
        elif affection >= 20:
            if choice == "breast":
                yield ui.dialog([
                    "손을 뻗어 유키의 가슴에 닿으려는 순간—",
                    "[유키]",
                    "+...!!! 저, 저기...!",
                    "+그건... 좀...",
                    "유키가 겁먹은 눈으로 이불을 끌어당겼다."
                ])
            elif choice == "butt":
                yield ui.dialog([
                    "손을 뻗어 유키의 엉덩이에 닿으려는 순간—",
                    "[유키]",
                    "+히...!",
                    "+...안 돼요...",
                    "유키가 떨리는 목소리로 거부했다."
                ])
            elif choice == "kiss":
                yield ui.dialog([
                    "유키의 얼굴에 가까이 다가갔다.",
                    "[유키]",
                    "+...너무 가까워요...",
                    "+...아직...",
                    "유키가 얼굴을 숙이며 뒤로 물러났다."
                ])
            elif choice == "hug":
                yield ui.dialog([
                    "유키를 안으려 했다.",
                    "[유키]",
                    "+...어...!",
                    "+...저... 좀 놀랐어요...",
                    "유키가 뻣뻣하게 굳어 있다."
                ])
        else:
            action_text = ""
            if choice == "breast":
                action_text = "손을 뻗어 유키의 가슴에 닿으려는 순간—"
            elif choice == "butt":
                action_text = "손을 뻗어 유키의 엉덩이에 닿으려는 순간—"
            elif choice == "kiss":
                action_text = "유키의 얼굴에 가까이 다가가려는 순간—"
            elif choice == "hug":
                action_text = "유키를 안으려는 순간—"
            yield ui.dialog([
                action_text,
                "[유키]",
                "+...! ...하지 마세요...!",
                "+(유키가 몸을 웅크리며 떨기 시작했다.)",
                "...너무 겁을 먹은 것 같다.",
                "+그만두는 게 좋겠다."
            ])

    def on_bed_sleeping(self, bed, player_id, slot, affection, owner_id):
        """유키가 자고 있을 때 - 호감도별 묘사 + 행동 선택"""
        success = False
        if affection >= 50:
            yield ui.dialog([
                "유키가 조용히 잠들어 있다.",
                "작은 체구가 이불 속에 꼭 감싸여 있다."
            ])
            success = morld.sit_on(player_id, bed.instance_id, slot)
            if success:
                yield ui.dialog(["조심스럽게 옆에 누웠다."])
        elif affection >= 20:
            yield ui.dialog([
                "유키가 잠들어 있다.",
                "잠결에도 불안한 표정이다."
            ])
            success = morld.sit_on(player_id, bed.instance_id, slot)
            if success:
                yield ui.dialog(["깨우지 않게 살짝 옆에 누웠다."])
        else:
            yield ui.dialog([
                "유키가 잠들어 있다.",
                "...엘라가 알면 죽을 수도 있다."
            ])
            success = morld.sit_on(player_id, bed.instance_id, slot)
            if success:
                yield ui.dialog(["숨을 죽이며 옆에 누웠다."])

        if not success:
            return

        # 수면 중 행동 선택지
        choice = yield ui.dialog(
            "...\n\n"
            "[url=@ret:breast]가슴 만지기[/url]\n"
            "[url=@ret:butt]엉덩이 만지기[/url]\n"
            "[url=@ret:kiss]키스하기[/url]\n"
            "[url=@ret:nothing]가만히 있기[/url]",
            autofill="off"
        )

        if choice == "nothing" or not choice:
            return

        if choice == "breast":
            if affection >= 50:
                yield ui.dialog([
                    "손을 뻗어 유키의 가슴에 살짝 닿았다.",
                    "+...작고 부드럽다.",
                    "유키가 잠결에 \"음...\" 하고 작게 신음했다.",
                    "+얼굴이 살짝 붉어졌다.",
                    "+...깨지 않았다."
                ])
            else:
                yield ui.dialog([
                    "손을 뻗어 유키의 가슴에 살짝 닿았다.",
                    "+...작고 부드럽다.",
                    "유키가 잠결에 몸을 움츠렸다.",
                    "+불안한 표정이 스쳤다.",
                    "+...그만두는 게 좋겠다."
                ])
        elif choice == "butt":
            if affection >= 50:
                yield ui.dialog([
                    "손을 뻗어 유키의 엉덩이에 살짝 닿았다.",
                    "+...작고 부드럽다.",
                    "유키가 잠결에 살짝 몸을 뒤척였다.",
                    "+\"...음...\"",
                    "+...깨지 않았다."
                ])
            else:
                yield ui.dialog([
                    "손을 뻗어 유키의 엉덩이에 살짝 닿았다.",
                    "+...작고 부드럽다.",
                    "유키가 잠결에 몸을 떨었다.",
                    "+...이 이상은 너무하다."
                ])
        elif choice == "kiss":
            if affection >= 50:
                yield ui.dialog([
                    "유키의 얼굴에 가까이 다가갔다.",
                    "잠든 유키의 이마에 살짝 키스했다.",
                    "+유키가 잠결에 살짝 미소 짓는 것 같다.",
                    "\"...음... 따뜻해...\"",
                    "+...깨지 않았다."
                ])
            else:
                yield ui.dialog([
                    "유키의 얼굴에 가까이 다가갔다.",
                    "잠든 유키의 이마에 가볍게 키스했다.",
                    "+유키의 눈꺼풀이 떨렸다.",
                    "...깨기 전에 그만두자."
                ])


# ========================================
# AI Agent
# ========================================

@register_agent_class("yuki")
class YukiAgent(BaseAgent):
    """
    유키 AI - 도심 은신처 생활

    특징:
    - 수줍고 얌전함
    - 은신처에서 조용히 지냄
    - 엘라를 의지함
    """

    # 도심 은신처 스케줄 (region_id=2, location_id=5=은신처, length=180)
    SCHEDULE = [
        {"name": "목욕", "region_id": 2, "location_id": 5, "x": 150, "start": 420 * _M, "end": 450 * _M, "activity": "목욕"},
        {"name": "기상", "region_id": 2, "location_id": 5, "x": 90, "start": 450 * _M, "end": 480 * _M, "activity": "준비"},
        {"name": "아침식사", "region_id": 2, "location_id": 5, "x": 90, "start": 480 * _M, "end": 540 * _M, "activity": "식사"},
        {"name": "청소", "region_id": 2, "location_id": 5, "x": 60, "start": 540 * _M, "end": 660 * _M, "activity": "청소"},
        {"name": "오전활동", "start": 660 * _M, "end": 720 * _M, "dynamic": True, "candidates": [
            {"activity": "요리", "condition": "can_cook"},
            {"activity": "독서", "condition": None, "region_id": 2, "location_id": 5, "x": 120},
        ]},
        {"name": "점심식사", "region_id": 2, "location_id": 5, "x": 90, "start": 720 * _M, "end": 780 * _M, "activity": "식사"},
        {"name": "정원", "region_id": 2, "location_id": 5, "x": 160, "start": 780 * _M, "end": 900 * _M, "activity": "정원"},
        {"name": "휴식", "region_id": 2, "location_id": 5, "x": 120, "start": 900 * _M, "end": 1020 * _M, "activity": "휴식"},
        {"name": "저녁식사", "region_id": 2, "location_id": 5, "x": 90, "start": 1080 * _M, "end": 1140 * _M, "activity": "식사"},
        {"name": "독서", "region_id": 2, "location_id": 5, "x": 120, "start": 1140 * _M, "end": 1320 * _M, "activity": "휴식"},
        {"name": "수면", "region_id": 2, "location_id": 5, "x": 120, "start": 1320 * _M, "end": 420 * _M, "activity": "수면"},
    ]

    owner_unique_id = "yuki"
    sleep_location = {"region_id": 2, "location_id": 5, "x": 50}  # 은신처 유키침대
    bath_location = {"region_id": 2, "location_id": 5, "x": 150}  # 은신처 드럼통
    food_storage_location = {"region_id": 2, "location_id": 5, "x": 120}  # 은신처 식량함
    food_storage_unique_id = "food_storage"
    wardrobe_location = {"region_id": 2, "location_id": 6, "x": 120}  # 의류점 옷걸이
    toilet_location = {"region_id": 2, "location_id": 5, "x": 170}  # 은신처 간이 화장실

    def __init__(self, unit_id):
        super().__init__(unit_id)
        self.set_base_schedule(self.SCHEDULE)
        import survival
        survival.register_npc(unit_id)
        import temperature
        temperature.register_character(unit_id)
        import needs
        needs.register_character(unit_id)
        import pregnancy
        pregnancy.register_character(unit_id)

    def on_leave(self, region_id, location_id):
        """위치 이탈 시 조명 끄기 (유키 성격: 꼼꼼함)"""
        self._turn_off_lights_here(region_id, location_id)
