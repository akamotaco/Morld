...
def restore_player_data(data):
    ...
    if "props" in data and data["props"]:
        ...
        for prop_name, value in data["props"].items():
            # 챕터 전환 시 복원하지 않는 props:
            # - posture/seated_on: 이전 챕터의 오브젝트 ID가 유효하지 않을 수 있음
            # - stance: 이동 모드 초기화 (걷기로 리셋)
            # - status:stealth: 은신 상태 초기화
            # - 상태:탈진: survival.reset()이 내부 상태를 초기화
            if (prop_name.startswith("posture:") or
                prop_name.startswith("seated_on:") or
                prop_name.startswith("stance:") or
