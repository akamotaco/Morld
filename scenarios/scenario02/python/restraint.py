# restraint.py - 구속 시스템 API
#
# 구속 장비(로프, 수갑 등)에 의한 행동 제한 관리.
#
# 구속 상태 props:
#   - "구속:사지": 1  → 이동/아이템/착탈의/식사/자위 등 행동 불가
#   - "구속:입": 1    → 말하기/구강행위/식사/소리치기 불가
#   - "구속:눈": 1    → 시각 차단 (감각 증폭 효과)
#   - "구속:강도": N  → 해제 난이도 (높을수록 자력 해제 어려움)
#
# 구속 중 가능한 행동 (NPC think):
...
def is_restrained(unit_id):
    """사지 구속 여부"""
    return bool(morld.get_unit_prop(unit_id, "구속:사지"))


def is_gagged(unit_id):
    """입 구속 여부"""
    return bool(morld.get_unit_prop(unit_id, "구속:입"))


def is_blindfolded(unit_id):
    """시각 차단 여부"""
    return bool(morld.get_unit_prop(unit_id, "구속:눈"))


def get_restraint_strength(unit_id):
    """구속 강도 (해제 난이도)"""
    return morld.get_unit_prop(unit_id, "구속:강도") or 0


def is_any_restrained(unit_id):
    """사지/입/눈 중 하나라도 구속된 상태인지"""
...
def attempt_restrain(actor_id, target_id, item_id, mode=None):
    ...
    if actor_id == target_id:
        ...
        return True, "구속 장비를 장착했다."

    # 의식불명/시간정지 — 무조건 성공
    if mode in ("unconscious", "frozen"):
        _apply_restraint(target_id, item_id)
        return True, "저항 없이 구속 장비를 채웠다."
    ...
    if mode == "consensual" or mode is None:
        ...
        if affection >= 70 or submission >= 50:
            _apply_restraint(target_id, item_id)
            return True, "상대가 순순히 구속을 받아들였다."
        return False, "상대가 구속을 거부했다."
    ...
    if mode == "forced":
        ...
        if random.random() >= block_chance:
            _apply_restraint(target_id, item_id)
            return True, "저항을 제압하고 구속 장비를 채웠다."
        return False, "상대가 강하게 저항하여 실패했다."

    return False, "구속할 수 없는 상태다."


def _apply_restraint(target_id, item_id):
    """구속 장비 장착 적용 — equip 시스템 사용"""
...
def release_unit(unit_id):
    ...
    for item_id in list(equipped_items):
        ...
        # 구속 장비인지 확인 (구속:사지 또는 구속:입 또는 구속:눈)
        if ep.get("구속:사지") or ep.get("구속:입") or ep.get("구속:눈"):
...
def release_self(unit_id):
    ...
    for item_id in list(equipped_items):
        ...
        if ep.get("구속:사지"):
            equipment.unequip_item(unit_id, item_id)
            break  # 사지 구속 장비는 하나만 가능


def release_mouth(unit_id):
    """입 구속 해제 (사지 자유 상태에서만 호출)"""
    equipped_items = equipment.get_equipped_items(unit_id)
    for item_id in list(equipped_items):
        item_info = morld.get_item_info(item_id)
        if not item_info:
...
def get_restrained_units_at(location_id):
    """
    특정 location에 있는 구속된 유닛 목록

    Returns:
        list[int]: 구속된 유닛 ID 리스트
    """
