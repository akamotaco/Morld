...
GATES = [
    # === 저택 1층 연결 ===
    # 현관(0) <-> 거실(1)
    (REGION_ID, 0, 0, 180, REGION_ID, 1, 0),   # 현관 끝(x=180) -> 거실 입구(x=0)
    (REGION_ID, 1, 0, 0, REGION_ID, 0, 180),   # 거실 입구(x=0) -> 현관 끝(x=180)

    # 거실(1) <-> 주방(2)
    (REGION_ID, 1, 1, 300, REGION_ID, 2, 0),   # 거실 오른쪽(x=300) -> 주방 입구(x=0)
    (REGION_ID, 2, 0, 0, REGION_ID, 1, 300),   # 주방 입구(x=0) -> 거실 오른쪽(x=300)

    # 거실(1) <-> 식당(3)
    (REGION_ID, 1, 2, 150, REGION_ID, 3, 0),   # 거실 중앙(x=150) -> 식당 입구(x=0)
    (REGION_ID, 3, 0, 0, REGION_ID, 1, 150),   # 식당 입구(x=0) -> 거실 중앙(x=150)

    # 거실(1) <-> 욕실(4)
    (REGION_ID, 1, 3, 240, REGION_ID, 4, 0),   # 거실(x=240) -> 욕실 입구(x=0)
    (REGION_ID, 4, 0, 0, REGION_ID, 1, 240),   # 욕실 입구(x=0) -> 거실(x=240)

    # 거실(1) <-> 주인공 방(6)
    (REGION_ID, 1, 4, 60, REGION_ID, 6, 0),   # 거실 왼쪽(x=60) -> 주인공 방(x=0)
    (REGION_ID, 6, 0, 0, REGION_ID, 1, 60),   # 주인공 방(x=0) -> 거실(x=60)

    # 거실(1) <-> 리나 방(7)
    (REGION_ID, 1, 5, 90, REGION_ID, 7, 0),   # 거실(x=90) -> 리나 방(x=0)
    (REGION_ID, 7, 0, 0, REGION_ID, 1, 90),   # 리나 방(x=0) -> 거실(x=90)

    # 거실(1) <-> 밀라 방(9)
    (REGION_ID, 1, 6, 120, REGION_ID, 9, 0),   # 거실(x=120) -> 밀라 방(x=0)
    (REGION_ID, 9, 0, 0, REGION_ID, 1, 120),   # 밀라 방(x=0) -> 거실(x=120)

    # 거실(1) <-> 1층 화장실(15)
    (REGION_ID, 1, 7, 270, REGION_ID, 15, 0),  # 거실(x=270) -> 1층 화장실(x=0)
    (REGION_ID, 15, 0, 0, REGION_ID, 1, 270),  # 1층 화장실(x=0) -> 거실(x=270)

    # 주방(2) <-> 식당(3)
    (REGION_ID, 2, 1, 180, REGION_ID, 3, 180),  # 주방 끝(x=180) -> 식당(x=180)
    (REGION_ID, 3, 1, 180, REGION_ID, 2, 180),  # 식당(x=180) -> 주방 끝(x=180)

    # === 저택 2층 연결 ===
    # 거실(1) <-> 2층 복도(14) - 계단
    (REGION_ID, 1, 8, 180, REGION_ID, 14, 0),  # 거실 계단(x=180) -> 2층 복도(x=0)
    (REGION_ID, 14, 0, 0, REGION_ID, 1, 180),  # 2층 복도(x=0) -> 거실 계단(x=180)

    # 2층 복도(14) <-> 세라 방(8)
    (REGION_ID, 14, 1, 60, REGION_ID, 8, 0),  # 2층 복도(x=60) -> 세라 방(x=0)
    (REGION_ID, 8, 0, 0, REGION_ID, 14, 60),  # 세라 방(x=0) -> 2층 복도(x=60)

    # 2층 복도(14) <-> 빈 방 1(10)
    (REGION_ID, 14, 2, 120, REGION_ID, 10, 0), # 2층 복도(x=120) -> 빈 방 1(x=0)
    (REGION_ID, 10, 0, 0, REGION_ID, 14, 120), # 빈 방 1(x=0) -> 2층 복도(x=120)

    # 2층 복도(14) <-> 빈 방 2(11)
    (REGION_ID, 14, 3, 180, REGION_ID, 11, 0), # 2층 복도(x=180) -> 빈 방 2(x=0)
    (REGION_ID, 11, 0, 0, REGION_ID, 14, 180), # 빈 방 2(x=0) -> 2층 복도(x=180)

    # 2층 복도(14) <-> 창고(5)
    (REGION_ID, 14, 4, 240, REGION_ID, 5, 0),  # 2층 복도(x=240) -> 창고(x=0)
    (REGION_ID, 5, 0, 0, REGION_ID, 14, 240),  # 창고(x=0) -> 2층 복도(x=240)

    # 2층 복도(14) <-> 2층 화장실(16)
    (REGION_ID, 14, 5, 300, REGION_ID, 16, 0), # 2층 복도(x=300) -> 2층 화장실(x=0)
    (REGION_ID, 16, 0, 0, REGION_ID, 14, 300), # 2층 화장실(x=0) -> 2층 복도(x=300)
    ...
    # 현관(0) <-> 뒷마당(13)
    (REGION_ID, 0, 2, 90, REGION_ID, 13, 0),  # 현관 옆(x=90) -> 뒷마당(x=0)
    (REGION_ID, 13, 0, 0, REGION_ID, 0, 90),  # 뒷마당(x=0) -> 현관 옆(x=90)
    ...
    # 숲 입구(20) <-> 강가(22)
    (REGION_ID, 20, 2, 1200, REGION_ID, 22, 0),  # 숲 입구 옆(x=1200) -> 강가(x=0)
    (REGION_ID, 22, 0, 0, REGION_ID, 20, 1200),  # 강가(x=0) -> 숲 입구(x=1200)

    # 숲 입구(20) <-> 채집터(23)
    (REGION_ID, 20, 3, 900, REGION_ID, 23, 0),  # 숲 입구(x=900) -> 채집터(x=0)
    (REGION_ID, 23, 0, 0, REGION_ID, 20, 900),  # 채집터(x=0) -> 숲 입구(x=900)

    # 숲 깊은 곳(21) <-> 사냥터(24)
    (REGION_ID, 21, 1, 1800, REGION_ID, 24, 0),  # 숲 깊은 곳(x=1800) -> 사냥터(x=0)
    (REGION_ID, 24, 0, 0, REGION_ID, 21, 1800),  # 사냥터(x=0) -> 숲 깊은 곳(x=1800)

    # 채집터(23) <-> 강가(22)
    (REGION_ID, 23, 1, 900, REGION_ID, 22, 1200), # 채집터(x=900) -> 강가(x=1200)
    (REGION_ID, 22, 1, 1200, REGION_ID, 23, 900), # 강가(x=1200) -> 채집터(x=900)
]
