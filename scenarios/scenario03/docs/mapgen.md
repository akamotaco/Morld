...
| 항목 | 상태 |
|------|------|
| BSP 알고리즘 | **구현 완료** (`mapgen.py`) |
| 난이도 프리셋 (easy/normal/hard) | **구현 완료** |
| Region/Location/Gate 생성 | **구현 완료** |
| 위협/전리품 배치 | **구현 완료** (room dict에 저장) |
| 시드 기반 재현 | **구현 완료** (`random.seed()`) |
| 탐사 완료 후 정리 | **구현 완료** (`cleanup_expedition()`) |
| 2D 시각화 | 미구현 (아래 설계 참조) |
| Gate 조건 (잠긴 문 등) | 미구현 |
...
게임에는 세 종류의 지형 그래프가 존재하며, 각각 2D 배치 특성이 다르다.

| 지형 | 생성 방향 | 그래프 구조 | 2D 좌표 |
|------|----------|-----------|---------|
| 탐사 맵 (mapgen) | 2D → 1D | 일반 그래프 (사이클 가능) | BSP가 생성 (보유) |
| 건축 (build) | 1D → 2D | 트리 (사이클 없음) | 없음 (역산 필요) |
| S02 기존 맵 | 수동 정의 | 일반 그래프 | 없음 |

### 탐사 맵: 문제 없음

BSP가 rooms에 x, y, width, height를 이미 보유. 그대로 렌더링 가능.

```python
# mapgen.py의 room dict
{"id": 0, "x": 0, "y": 0, "width": 200, "height": 150, "type": "entrance"}
```

### 건축 맵: 트리 레이아웃

`build_location_frame()`은 항상 **새 방 1개 + Gate 1쌍**을 생성하므로,
건축 결과물은 항상 **트리(tree)**이다. 사이클이 발생하지 않는다.
...
**트리는 항상 평면 그래프** → 교차 없이 2D 배치 가능.

단, 고정 오프셋 배치(위/아래 교대)는 분기가 깊어지면 파탄:

```
통로(L1) ─┬─ A ─┬─ E
          │     └─ F ─── H
          ├─ B ─── G        ← E, F, G가 A, B 영역과 겹침
          └─ C
```

**해결: Reingold-Tilford 트리 레이아웃**
1. 리프 → 루트: 각 노드의 서브트리 폭 계산
2. 루트 → 리프: 좌표 할당 (형제 간 간격 = 서브트리 폭)
3. 겹침이 원천적으로 불가능

좌표를 Location에 저장할 필요 없음 — **렌더링 시점에 매번 재계산**.
건축이 추가될 때마다 전체 트리를 재배치.

### 기존 방 간 Gate 추가 (사이클 발생) 시

현재 `build_location_frame()`에는 기존 방끼리 연결하는 API가 없다.
향후 추가되더라도:
- 플랫폼 규모(10개 미만 노드)에서 비평면 그래프 출현 가능성 극히 낮음
- 필요 시 Force-Directed 레이아웃으로 전환
- **현재는 트리 전제로 설계, 사이클 지원은 필요 시점에 구현**

### S02 기존 맵

시나리오02 `map_ui.py`는 BFS 텍스트 트리 렌더링.
사이클이 있는 그래프도 tree로 펼치므로 공간 관계 소실.

S02 `map.md`에 Force-Directed 레이아웃 설계가 있으나 미구현.
S02/S03 공용 레이아웃 엔진으로 구현하면 세 경우를 모두 커버 가능:
- Force-Directed는 트리에도, 사이클 그래프에도, BSP 초기값에도 동작

### 통합 레이아웃 전략

```
입력 그래프 → 초기 좌표 있음? ─ Yes → BSP 좌표 사용 (탐사 맵)
                              └ No → 트리? ─ Yes → Reingold-Tilford (건축)
                                            └ No → Force-Directed (일반)
```

구현 시점: **CRT 뷰 UI (C#) 구현 시 함께 진행**.
Python에서는 좌표 계산까지만, C#에서 렌더링.
