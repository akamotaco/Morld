# build.py - 건축/파괴 시스템 (시나리오03)
#
# 시나리오02와 동일한 인터페이스를 공유한다.
# 추가 기능: 원격 건축 지정 (designate_build), 데모 레시피 등록
#
...
class BuildRecipe:
    ...
    def __init__(self, unique_id, name, recipe_type="location",
                 tool_category=None, materials=None, result_class=None,
                 base_length=50, progress_per_build=10, indoor=True):
...
def build_location_frame(builder_id, source_region, source_location, gate_x,
    ...
    morld.add_location(
        ...
        indoor=indoor, length=base_length,
    )

    # 양방향 gate
    src_gate_id = _next_gate_id(source_region, source_location)
    new_gate_id = _next_gate_id(source_region, new_local_id)
    ...
    from assets.objects import register_instance, register_location_object

    site = ConstructionSite()
    site_id = morld.create_id("unit")
    site.instantiate(site_id, source_region, new_local_id, x=0)
    register_instance(site_id, site)
    ...
    print(f"[build] Frame created: {name} at R{source_region}:L{new_local_id}"
          f" (site_id={site_id})")
...
def build_location_progress(builder_id, site_id, materials_used=None):
    ...
    Args:
        ...
        materials_used: [(item_uid, count), ...] 투입할 재료 (None이면 무자재 진행)
...
def destroy_object(destroyer_id, object_id):
    ...
    if owner and owner != destroyer_name and owner != "operator":
